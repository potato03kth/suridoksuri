...
> 🆕 **2026-07-30 — CSI 카메라는 물리적 사망으로 확정됐고(리본 재체결·재부팅 실패),
> USB 웹캠(UVC) 임시 경로가 생겼다.** 실촬영이 다시 가능하다:
> **`docs/vision_webcam_fallback.md`** (구매 체크리스트 → 화각 실측 → `nominal.yaml` 생성 → 실행).
> 아래 §2 "경로 A"의 `live`를 `uvc`로 바꾸면 이 문서의 나머지 절차가 그대로 적용된다.
> 중간에 검토했다 접은 휴대폰 IP Webcam 안은 화질 부족으로 기각됐다.
