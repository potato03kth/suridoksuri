...
- ~~**서브에이전트는 소네트로 생성한다.**~~ **[2026-07-25 폐기]** 당시엔 월 사용한도에 걸려 에이전트가 중단된 이력 때문에 둔 제약이었으나, 사용자가 Claude Max를 구독해 해소됐다. **서브에이전트 model/effort는 미지정(하네스 기본)이 기본값**이며, 사용자가 요청하면 그때 그대로 따른다.
