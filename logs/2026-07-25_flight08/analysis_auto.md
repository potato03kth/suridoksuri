# 자동분석 — logs/2026-07-25_flight08

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_170_2026-07-25-10-23-24.ulg` (25.69s)
...
### 고도
- home_position AMSL=24.4m, EKF ref_alt AMSL=22.65m
- AGL 최대 3.17m (t=16.57s), 최소 -1.9m (t=25.67s), 종료시 -1.9m
...
| 3.45 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 13.7 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 17.15 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=25.91° (t=14.87s)
- pitch 최대|값|=-31.27° (t=19.47s)
- roll 목표(setpoint) 최대|값|=32.49° (t=11.91s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=6.97s (-11.6deg/s)
- pitch rate 이상조짐 후보: t=6.54s (9.9deg/s)
- yaw rate 이상조짐 후보: t=7.08s (9.8deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.608 max=1.0 min=0.002
- 1:후좌: mean=0.592 max=1.0 min=0.0
- 2:전좌: mean=0.45 max=1.0 min=0.0
- 3:후우: mean=0.618 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.39V, 부하중 최저 12.29V, 전류 최대 35.1A
- remaining: 0.84 → 0.59

### 실패감지기
- imbalanced_prop_metric_max_abs: -4.003
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=4.39s
