...
- **마지막:** **⚠(2026-07-20, flight01 — 오프보드 전환 직후 제어상실 사고, 원인 미분석)** `climbing_reached` 허용오차 수정 후 실비행(`transition_alt:=4.0`, 삼각 왕복). launch.log만 보면 ARM(16:18:50)→이륙→CLIMBING(4.0m, 16:19:00)→**OFFBOARD 확인(16:19:03)**→FOLLOWING→WP1 도달→LANDING→착륙완료(16:19:29, 총 39초)로 "정상 완주"처럼 보여 Claude가 대화 중 **"첫 오프보드 성공"으로 잘못 판단**했으나, **조종사(사용자) 직접 증언으로 정정: 수직 상승 완료 직후(오프보드 전환 시점 전후로 추정, 16:19:03 부근) 기체가 순간 제어를 잃고 북서쪽으로 픽 쓰러지며 roll, 즉시 조종사가 RC로 조종권 회수.** 즉 **16:19:03 이후 log의 FOLLOWING→WP1→LANDING 시퀀스(26초)는 실제 자율비행 수행 기록이 아닐 가능성이 높음** — 조종사가 수동 회수·착륙시킨 실제 위치를 소프트웨어가 그대로 읽고 우연히 거리/안정 임계값을 만족해 상태머신만 진행된 것으로 추정(미확정). **원인 분석은 별도 세션에서 진행 예정 — 이 항목은 기록 전용.** 데이터 가용성: ①**ulog 회수 완료(갱신: 최초엔 실패했으나 사용자가 Pixhawk 전원을 재연결해줘서 재시도 성공)** — FC 로그 목록에 오늘자 3건 확인: id16(07:18:28 UTC, 156,843B)·id17(07:18:28 UTC, 156,277B, id16과 같은 초에 별도 로그 — 재시동/재arm 추정, 원인 미확인)·**id18(07:19:30 UTC, 1,729,984B — 가장 크고 이 비행 전체를 담고 있을 가능성 높은 본 로그)**, UTC+9=KST로 정확히 이 비행 시각대(16:18~16:19 KST)와 일치. 3개 전부 `logs/2026-07-20_flight01/log_{16,17,18}_...ulg`로 다운로드 완료. **pyulog가 RPi에 미설치라 내용 분석은 안 함**(다음 세션에서 `pip install pyulog` 후 분석 필요 — **roll/pitch/각속도(vehicle_attitude, vehicle_angular_velocity 등)가 이 사고의 핵심 증거**). ②**rosbag엔 `/mavros/imu/data` 포함**(자세 쿼터니언·각속도 있음 — ulog 분석 전이라도 이걸로 roll 이벤트 자체는 먼저 확인 가능) ③**`/fc_ros/override` 토픽이 이번 rosbag엔 기록 안 됨**(설정된 11개 중 10개만 실제 기록 — RC 오버라이드 개입 시점을 이 토픽으로 직접 못 잡음, ulog `vehicle_command`/`manual_control_setpoint` 또는 `/mavros/state` 모드 전이로 간접 추정 필요) ④**wifi_watch.log 참고자료(인과관계 미확정):** 게이트웨이 ping 무응답이 비행 시작 전 16:15경부터 이미 간헐 발생 중이었고, wlan0 `carrier=0`(인터페이스 완전 다운)은 16:20:38~16:26:14(약 5분 36초)로 **사고 시점(16:19경)보다 약 1분 뒤에 시작** — 타이밍상 사고와 직접 겹치진 않으나 사용자가 "중간에 끊겼다"고 보고한 것과 일치. flight01 폴더(`logs/2026-07-20_flight01/`, ulog 3개 포함)·`notes.md`(조종사 증언 반영해 갱신함)에 원본 전부 보존. **다음 세션 필수 확인:** (a) **id18 ulog를 pyulog로 분석 — `vehicle_attitude`(roll/pitch/yaw)·`vehicle_angular_velocity`·`vehicle_local_position`으로 16:19:03(=UTC 07:19:0x) 전후 실제 roll 이벤트·자세 급변 구간 특정** (b) id16·id17(같은 초 156KB 2건)이 무엇을 기록한 로그인지 확인(이륙 전 짧은 arm 시도? 로거 재시작?) (c) `vehicle_command`/`manual_control_setpoint`로 RC 오버라이드 개입 정확한 시각 특정, rosbag `/mavros/state` 모드 전이와 교차검증 (d) `MIS_TAKEOFF_ALT` 등 PX4 파라미터도 이 ulog로 조회 가능해졌으니 flight09 잔여 미확정 사항과 함께 확인 (e) 이 사고가 위 ✈vtol-실기체 트랙의 과거 결함과 연관 있는지는 미검토.
...
- **주의:** AUTO.TAKEOFF는 GPS 락 필수(실내/벤치 불가) · 실기체 FC는 PX4인지 확인부터 · **비행 전 SD카드 삽입 확인 (2026-07-07 이걸로 비행 실패 이력)** · **비행 중 tailscale/WiFi 간헐적 끊김 가능성 있음**(위 진단 참조 — RC/Pixhawk 텔레메트리 링크와는 무관, SSH/tailscale 원격접속 편의 채널만의 문제) · **⚠flight01(2026-07-20) 오프보드 전환 직후 제어상실·roll 사고 발생 — ulog 3개(id16/17/18) 회수 완료했으나 분석은 미착수. 다음 실비행 전 반드시 위 "마지막" 항목 확인, launch.log의 겉보기 "정상완주" 기록을 그대로 신뢰하지 말 것**
