# 자동분석 — logs/2026-07-30_manual

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_266_2026-07-30-01-12-46.ulg` (46.0s)
...
### 고도
- home_position AMSL=18.87m, EKF ref_alt AMSL=19.72m
- AGL 최대 3.98m (t=28.57s), 최소 -2.01m (t=40.97s), 종료시 -1.95m
...
### 자세
- roll 최대|값|=20.19° (t=28.67s)
- pitch 최대|값|=-19.17° (t=22.77s)
- roll 목표(setpoint) 최대|값|=21.57° (t=28.37s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=9.6s (-9.1deg/s)
- pitch rate 이상조짐 후보: t=10.5s (-8.9deg/s)
- yaw rate 이상조짐 후보: t=9.45s (8.7deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.487 max=0.965 min=0.002
- 1:후좌: mean=0.376 max=0.723 min=0.0
- 2:전좌: mean=0.456 max=0.8 min=0.002
- 3:후우: mean=0.448 max=0.786 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.76V, 부하중 최저 13.7V, 전류 최대 25.1A
- remaining: 0.98 → 0.75

### 실패감지기
- imbalanced_prop_metric_max_abs: -4.871
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=8.41s
