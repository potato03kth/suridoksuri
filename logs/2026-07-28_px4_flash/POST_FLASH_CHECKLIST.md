...
#### ✅ 이 절차의 유효성 — 확인됨 (2026-07-29)

**이 체크리스트가 실제로 사고를 잡아낸 절차다.** §1 판정표(`ver all`)는 "펌웨어가 바뀌었다"까지만
보증하고 **RC 두절을 전혀 잡지 못했다** — 사고를 드러낸 것은 오직 §2-2 의 `MISSING` 2건이었다.
2차 플래시 후 같은 절차를 그대로 돌려 **해소까지 같은 방식으로 확정**했다
(원본 대비 MISSING 0건 / 사고본 대비 `RC_CRSF_*` ADDED 2건 복귀, 개수 1436+2=1438 정합 —
`docs/px4_v6c_patch_build.md` §11-6c).

> ⇒ **플래시 후 파라미터 대조는 선택이 아니라 필수 절차다.** `ver all` 만으로 끝내지 말 것.
...
## 3. 자기계 — 🔴 **이 절의 원래 지시는 틀렸다 (2026-07-28 정정 + 2026-07-29 재정정 2건)**
...
**이 값의 출처 — 사용자가 직접 수동으로 재캘리브레이션한 것이다** (pre-flash 덤프(02:38) 이후
수행). 다만 18.8% 도 정상권(<10%)은 아니다.

> 🔴 **정정 (2026-07-29):** 이 자리에 원래 "사용자는 캘리브를 하지 않았다고 확인 —
> QGC 펌웨어 마법사가 건드렸을 가능성"이라고 적혀 있었다. **틀렸다.** 마법사가 아니라
> **사용자의 수동 재캘리브레이션**이 출처다. 위 `CAL_MAG*` 복원 금지 결론과
> `655f539` 대조표는 이 정정과 무관하게 **그대로 유효하다.**

> ✅ **조치: `CAL_MAG*` 복원 금지.** 백업(pre-flash)값이 곧 헤딩 65° 오차의 범인이므로
> 되돌리면 버그를 되살린다. 재캘리브 후에는 재덤프해서 백업을 갱신한다.

### 3-1. 🔴 재캘리브레이션은 **이미 수행됐다 — 그런데 미해결이다** (2026-07-29 갱신)

위 "비행 전 재캘리브레이션하라"는 권고는 **완료됐다**(위 post 실측값이 그 결과물이다).
그러나 후속 ulog 분석 `5d55b3f` 에서 **재캘리 후에도 지표가 오히려 악화**된 것이 드러났다:

| 지표 | 재캘리 전 (flight17, 07-25) | 재캘리 후 (flight02, 07-28) |
|---|---|---|
| `test_ratio` 평균 / 최대 | 1.97 / 3.32 | **2.62 / 6.69** |
| `cs_mag_fault` ON | 0.0 % | **92.7 %** |
| `field_disturbed` | 1.3 % | 16.8 % |

**원인은 전류 간섭이 아니라 헤딩 의존 오차(캘리 방향 커버리지)다** — `−45~0°` 구간에서
`test_ratio` 0.06 인데 `−180~−135°` 구간에서 4.18(최대 6.69). 이륙 헤딩은 좋은 구간이고,
임무의 천이 목표 헤딩(−161.6°)이 하필 최악 구간이었다.

> ✅ **통과 기준(갱신): 재캘리 후 기수를 남쪽에 두고 `test_ratio < 1` 을 확인한다.**
> "재캘리를 했다"만으로는 통과가 아니다. 근거·상세는 커밋 `5d55b3f`
