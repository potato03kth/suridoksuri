...
**✅ [2026-07-28] 재생성 경로 복구 완료.** 예전에는 `no_target/distress_coarse/`만 이 스크립트가
만들지 않아(2026-07-21 감사 세션이 손으로 추가) 재생성하면 그 리프가 조용히 사라졌다. 지금은
**커밋된 골든 20개 파일 전부가 바이트 동일하게 재생성된다**(복구 후 재생성 전후 md5 전량 일치 —
회귀 0). 손으로 만든 리프가 다시 생기지 않도록 `test_golden_regression.py::
test_every_golden_leaf_is_reproducible_by_generate_synthetic`이 **생성기 출력과 커밋된 골든을
바이트 단위로 대조**한다 — 리프 누락도, 생성기 출력의 조용한 드리프트도 red로 잡힌다.
