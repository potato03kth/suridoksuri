...
### 🎯 fc-정밀착륙 (F2) — ⏸ **SITL 완료·실비행 차단** (SITL 결함 c/d/f/g 전부 해소. **실비행 0건 — 천이가 안 돼 HOLD에 도달 못 한다. 차단 해제는 ✈ vtol-실기체 트랙의 지상 조치 2건**)

> # 이 트랙을 재개한다면 → **`docs/fc_precision_land_handoff.md` 하나만 읽으면 된다.**
> **단 사용자 결정 대기 항목의 정본은 `docs/fc_orchestration_handoff_2026-07-29.md`다**(2026-07-29 하루치
> 오케스트레이션 세션 6개를 이어받게 묶은 문서 — 결정 대기 4건 D-1~D-5, 완료 11건 커밋 대응표, 미해결,
> 함정 8건). 현장 실행 절차는 **`docs/field_vision_landing_2026-07-29.md`**(7차 개정본).
> 계약·붙는 자리·함정·**구현된 것과 안 된 것**·재개 순서가 전부 거기 있고, **vision 도메인
> 문서를 읽지 않아도 완주하도록** 썼다(도메인 컨텍스트 격리 유지).

- **✅ 2026-07-29 실기체 직전 SITL 검증 — F2 결함 c/d/f/g 전부 실증 + 후퇴 경로 완주 (`b9b8f8f`·`a6d2072`·`94ef01b`).** 격리 클론을 그날 나는 코드(`44027a5`)로 올려 `fc_ros` 재빌드 후 실행. **§2-2의 미실시 4건이 여기서 전부 닫혔다.**
  - **F2-f ✅** align 30s 타임아웃 → 나선 강제 진행(V6). **F2-c ✅** 생산자 사망을 t=13/60s에 잡아 GPS 폴백(V5b). **F2-g ✅** 인계고도 3.5m±0.5 정상 인계 — V2에서 보이던 "3.1m에서 22초 정지"가 사라졌다.
  - **F2-d ✅ (이 경로는 이번이 첫 실행)** `fake_vision --exit-at 10`으로 setpoint·status 동시 침묵 → **`vision_lost_timeout=5.0`이 실제로 발화한다.** 실측 5.83s(로그가 setpoint age=6.0s를 자백 = stale 1.0 + 상실 5.0), 전체 시한은 t=16/60s로 44초 남아 있었다(다른 시한이 먼저 잡은 게 아니다). GPS 착륙 폴백 → 21.4s 뒤 DONE, 호버 정지로 안 남는다. **폴백 직전 2.7초는 고도를 붙들었다**(17.7→16.4m, 하강 정지) — `descend_allowed(guided=False)`의 "추측 하강 금지" 계약대로다. **🔴 F2-c와 F2-d는 현장에서 로그로 구분된다:** F2-c = "생산자 사망(link ERROR)" / F2-d = "유도 상실 5s 지속"(link ERROR 한 줄도 없고 status age=6.0s가 증거 — `vision_target_bridge.py:74-77` 페일세이프 3분법이 설계대로 갈렸다).
  - **`vision_landing=false` 후퇴 경로 전용 런 ✅** — 인자를 아예 주지 않은(=yaml 기본 false, 현장에서 인자를 빼는 것과 동일) 상태로 **HOLD → LANDING → DONE 깨끗이 완주**(exit=0 done 119.5s). `VISION_SEARCH`/`PRECISION_LAND` 미등장, "비전 정밀착륙 활성" 줄 없음. **F2-e 계약 재확인:** 꺼져 있으면 거리상한 문구·판정이 무변경이다.
  - **짧은 경로 스윕(25/40/60m, 편도 150m 대조) — 현장 공간 제약 대응:** 상태기계는 **25m 편도까지 안 깨진다**(네 길이 전부 완주). FOLLOWING 체류가 8.2 → 2.4 → 1.0 → 0.2s로 사라져도(F-11/B7이 예고한 상태) 종점 포착이 **결승선 OR 거리원**(`8bbcb94`)이라 놓치지 않는다. `transition_alt=25`는 VISION_SEARCH 진입 **0.2~0.6s 만에** 래치시킨다. `range_limit_m` 기본 300은 어느 길이에서도 안전(실측 최대 112.6m).
  - **🔴 그러나 역천이 오버슈트가 ~70m에서 포화한다 — 편도 축소로 해결되지 않는다.** 경로를 줄여도 총 수평 이탈이 102~113m로 거의 안 준다. 현장 판정 기준은 "비행 방향 편도+75m, 횡 20m". 런 간 편차도 ±10m다(같은 60m 편도가 59.5m / 49.3m).
  - ⚠️ **재현 스크립트 형상 주의:** `f2c_v8.sh`/`f2c_v7.sh`/`f2c_v7b.sh`는 **앞 세션 형상(300m/alt50/상한1200)**이라 그대로 쓰면 안 된다 — 현재 형상판은 `f2c_v8b.sh`/`f2c_v7b.sh`다. 환경 함정: **gz sim이 안 뜨면 PX4가 6분 뒤 logger 스레드 segfault로 죽는다**(PX4 사망 2건 모두 코드와 무관).
- **🔴 운용 파라미터 3종 변경 (2026-07-29 사용자 지시, `fc8631d`) — 트랙보드의 옛 값(50m/300m/120s)은 전부 폐기됐다.**
  - **`transition_alt` 50.0 → 25.0.** 근거는 사용자 운용 현실("대회에서도 끽해야 20m, 무슨 일이 있더라도 30m가 최대")이고, 50.0이 결함 3건을 증폭하고 있었다: **F2 래치** — HOLD 49.6m에서 래치하면 하강만 58.2s라 `precision_land_timeout`(60s)를 거의 다 쓴다(25m면 27.5s) / **F-9** — 계단이 `wp[-1].z − transition_alt`라 둘을 25로 맞추면 0이 된다 / **탐색** — `vision_search_alt`가 25m인데 HOLD가 50m면 25m를 먼저 내려가야 했다. 검출 상한 33.57m 아래라 `vision_latch_max_agl` 게이트에도 자연히 들어온다. **낮출 수 있게 된 근거는 F-9 수정(`94989b6`)으로 천이 계단이 −29.6 → −8.3m로 줄어든 것** — 종전 계단폭이면 25m 천이는 지면에 위험했다.
  - **`waypoints` 300m 편도 → 150m 편도**, z도 25로 통일(`offboard_node`/`mission_node` yaml + 두 노드 declare 기본값).
  - **`vision_search_timeout` 120 → 180s.** 1회차 예산 실측이 나선 89.4 + 정렬 30.0 + dwell 3.0 = **122.4s로 이미 120을 초과**하고 있었다 — 정상 탐색이 타임아웃으로 끝날 수 있었다.
  - **테스트 3건이 낡은 값을 계약으로 고정하고 있어 함께 일반화했다**(F2-g와 같은 실패 모드 — 테스트가 결함을 보호한다): `test_waypoints_altitude_50m` → `..._matches_transition_alt`(절대값 50이 아니라 관계를 본다) / `test_new_params_present_in_yaml`(값 dict 고정 → 존재 검사) / `test_r1_climbing_timeout_...`(`approx(27.5)` 하드코딩 제거, 여유 배수만 계약). **테스트 783건 전건 통과.**
  - **미반영(설계 필요, D-5):** `range_limit_m` 300m 고정 해제. 지오펜스는 경로 길이에 비례해야 한다 — 다만 **긴급도는 하향**됐다(편도 25~150m 전 구간 실측 이탈이 102.5~188.0m라 300m가 안 걸린다). 구조 문제는 그대로다.
- **🔴 F2 실비행은 여전히 0건이다 (2026-07-29 11건·2026-07-30 7건 전수 확인).** 두 비행일의 어느 `launch.log`에도 `비전 정밀착륙 활성`·`VISION_SEARCH`·`PRECISION_LAND`가 **한 줄도 없다** = `vision_landing`이 켜진 적이 없다. **이유는 F2 코드가 아니라 그 앞단이다** — F2는 `HOLD` 이후에 발동하는데 ✈ vtol-실기체 트랙에서 보듯 **천이 자체가 성립하지 않아 HOLD에 도달하지 못했다**(07-30 f02는 피토 영점 21Pa 오차로 Quad-chute, 나머지는 목표고도 미달). **"SITL에서 c/d/f/g가 전부 닫혔다"를 "실기체에서 검증됐다"로 읽지 말 것.**
...
- **🔴 2026-07-29 후속 — 6건 + SITL 이 드러낸 7번째(F2-g). 전부 수정·배포됨**
  (`db7aa9c`·`3d73bfb`·`f3934e1`·`e530c0e`·`1999292`, 로그 `logs/2026-07-29_f2_followup/`).
  - **F2-g 가 가장 크다 — 해피패스의 "정상 종료"가 실은 타임아웃이었다.** `_pl_alt` 하한과
    `handoff_agl` 이 **같은 숫자(3.0 m)** 라 기체가 위치제어 오차만큼 그 위에 정착 →
    `agl <= 3.0` 이 **원리적으로 성립 불가**. ⇒ **고도 기반 인계는 한 번도 실행된 적이 없다.**
    앞선 완주도 `command_hint="land"` advisory 가 42s 에 구해준 것이었다. SITL 실측: AGL 3.4m 까지
    정상 하강 후 **3.1m 에서 22초 정지 → GPS 폴백**. 처방은 이 저장소가 이미 쓰던 것 —
    `climbing_reached()` 의 단측 판정 → `alt_tol` 구간 전환과 **같은 실패 모드·같은 숫자(0.5)**.
    ⚠️ **기존 테스트에 `handoff_due(3.1, 3.0) is False` 가 있어 결함이 계약으로 고정돼 있었다.**
  - **a/b(래치 고도 무제한 → 시한 초과)는 한 몸이라 둘 다 고쳤다:** `vision_latch_max_agl`
    게이트(0=검출상한 33.57m 자동) + 시한을 진입 시 `max(파라미터, 1.6 × 이상적 하강시간)` 으로
    재계산(**새 파라미터 없음**, `precision_land_timeout` 은 하한). SITL 실측: 래치가
    **50.0/42.3/34.7 m 에서 3회 거부되고 32.8 m 에서 성립**(종전엔 49.6 m 에서 그냥 래치).
    ⚠️ **배수 1.6 은 SITL 2런 실측이지 실비행 근거가 아니다** — 바람으로 정렬이 자주 끊기면 짧을 수 있다.
  - c/d(유도 상실): `link_dead` 판정 추가 + `vision_lost_timeout`(5.0s). **한 몸이다** — shim 자신이
    죽으면 `link_dead` 가 안 서므로 c 로는 못 잡고 d 가 메운다. e: 거리상한 경고가
    `d(WP1)+탐색반경` 까지 확인. f: `vision_align_timeout`(30s) 초과 시 dwell 건너뛰고 나선 강제 진행.
  - **미완(다음 세션 몫):** F2-c/d/f 의 SITL 실증(`V5`/`V6`)과 `vision_landing=false` 종점 완주 런.
    셋 다 **노드 테스트(상태기계 실제 실행)+파괴검증 11종 red→green** 으로는 덮였다. SITL 박스
    경합으로 미완이며 **재현 명령 3건이 `logs/2026-07-29_f2_followup/notes.md`** 에 있다
    (특히 `wp1_land_radius` 가 launch 인자로 **선언돼 있지 않아** F2-f 는 `v_approach=0.5` +
    `hold_timeout=20` 조합으로 만들어야 한다).
  - **🔴 배포 검증의 사각지대 발견:** `colcon build` 는 `fc_ros` 만 `install/` 로 옮기고
    **`fc_bridge` 는 `PYTHONPATH` 로 로드**된다(기본 셸에서 `import fc_bridge` 는 실패). 즉
    **md5 대조로는 `fc_bridge` stale 을 원리적으로 못 잡는다.** 오늘 `fc_bridge` 에 들어간
    `fw_setpoint_alt`(F-9)·`path_end_passed`(F-10)·`handoff_due(alt_tol)`(F2-g)가 이 사각지대에
    걸린다. 검증 명령을 `docs/preflight_physical_checklist.md` §5-b 로 박아뒀다
    (현재 실기체는 정상 — 세 함수 전부 존재 확인).
...
- **🔴 그러나 선행 조건이 새로 생겼다 — 실비행 검증은 아직 못 한다 (2026-07-29, **2026-07-31 갱신**):**
  - **🔴 2026-07-30 갱신 — 실제로 막고 있는 것은 아래 2건이 아니라 「천이가 안 된다」이다.** 07-29 11건·07-30 7건을 날렸지만 F2는 한 번도 켜지지 않았다(위 "F2 실비행은 여전히 0건" 항목). F2는 `HOLD` 이후에 발동하는데 거기까지 못 간다. **차단 원인은 2026-07-30에 규명됐고 처방이 두 줄이다** — 피토 영점 재캘리(`SENS_DPRES_OFF` −21.78 → −42.6 근방)와 `VT_ARSP_TRANS` 12.0 → 10.0. 상세는 ✈ vtol-실기체 트랙 "마지막". **그 지상 조치를 먼저 하지 않으면 아래 2건을 해결해도 F2 실비행은 못 한다.**
  - **🔴 그리고 추력 여유 자체가 없다(H19).** T/W 1.44라 7건 중 목표고도 도달이 1건뿐이었다 — F2 탐색·정렬은 그보다 위에서 도는 기동이다. CG 조정(H8, 비용 0)이 선행돼야 한다.
...
- **마지막:** **(2026-07-29, 인프라 3건 — 로그수집 확장 · 테스트 기준선 명문화 · 보고서 산출물)**
  ①**로그수집기 rosbag에 `/vision` 3종 추가**(`d4a8a76`) — F2 첫 실비행의 ROS 쪽 기록을 확보하려는 것. 다만 아래 🎯 F2 트랙에 적힌 대로 **그 첫 실비행은 아직 없었다**(07-29·07-30 전 비행에서 `vision_landing` 미활성).
  ②**테스트 기준선 783건 명문화**(`51b173e`, CLAUDE.md 반영) — `tools` 82건이 빠져 세션마다 594/617/646/701/736으로 숫자가 달랐던 것을 종결. 세는 명령을 CLAUDE.md에 고정했다(`__pycache__` 제거 후 4경로 합산). ⚠️ **07-29 F-5 세션은 아직 701건으로 보고했다**(`1da4174`에서 §12 표기만 783으로 정정) — 문서마다 낡은 숫자가 남아 있을 수 있다.
  ③**2차예선 보고서 산출물**(`7244318`·`b55dd06`·`f3e0f23`) — 러프 20매 골격 → HW/SW 비중 재배분 개정 → **PPT 23장 생성 + 생성 스크립트**(`docs/qualifier2_report_draft.pptx`, `docs/qualifier2_report_outline.md`, `docs/qualifier2_report_inventory.md`). 이어 07-30에 보고서용 이미지 생성기 2종 — 팀 조직도(`7b983bf`)와 온보드 시스템 구성도(`ff6f5e1`·`91a7f8a`·`03fefcb`, `tools/report/build_system_diagram.py` → `docs/figures/system_diagram.png`). 구성도는 실기체 `docker inspect`·`lsusb`·PX4 파라미터 실측으로 그렸다.
  그 전 **(2026-07-21) `tools/flight_logs/analyze_flight.py` 신설 — 비행 로그 표준 진단 스크립트.** flight02/03 사고분석 세션에서 매번 pyulog 코드를 새로 짜며 세션 컨텍스트를 소모하던 것을 대체. `<플라이트폴더>/` 하나 넘기면 고도/모드권한타임라인(조종기 vs 페일세이프 자동판별)/자세·각속도 이상조짐/축별 얼로케이터 포화시점/모터별 커맨드/배터리/실패감지기/이함순간을 `analysis_auto.md`(+`--json`)로 뽑아준다 — 해석은 안 하고 사실만. 다루지 않은 토픽은 목록으로 출력해 누락 방지. 순수 함수 22개 신규 pytest 케이스 추가, 전체 61개 통과(이 WSL에 pytest 설치함, 기존 "미설치" 메모는 아래 07-18 항목 기준 구버전 상태). 사용법은 `tools/flight_logs/README.md` §4. **다음:** 이 스크립트를 로그수집 서브에이전트/스킬과 엮어 "비행→수집→분석→평가" 사이클 자동화하는 게 목표(사용자 요청, 수십 회 반복 예정이라 세션 컨텍스트 보호 필요) — 로그수집 스킬은 아직 미착수.
...
### 🛩 sitl-vtol — ▶ 활성 (**2026-07-29: R5 F-9·F-10 + R6 F-5 ✅완료·SITL 검증. 남은 항목은 F-8·F-11·F-6·F-13**)

- **✅ R6 F-5 (선회 중 고도 침하) — 원인 규명 + 처방 SITL 검증 완료 (2026-07-29, `b3e35c7`·`47047ed`·`db4bb8c`). 전문 `docs/fc_f5_turn_altitude_loss.md`.**
  - **원인(코드 무변경 분석):** 우리 코드에 선회↔고도 연결점이 없다는 것은 사실이나(grep 0건) **결함 위치는 그보다 아래다.** PX4 TECS는 뱅크 보상항을 갖고 있고 실제로 작동했는데 **스로틀 쪽에만 있다**(`TECS.cpp:569-570`, `FW_T_RLL2THR`, 실측 +0.09) — 피치를 만드는 `_calcPitchControlSebRate`(`TECS.cpp:427-448`)는 `load_factor`를 참조하지 않는다. 침하의 **76~86%는 총에너지 손실이 아니라 고도→속도 배분 오류**(B4: Δh −6.63m 중 ΔKE +5.72m, 진짜 에너지 손실은 −0.91m). 스로틀은 어느 런에서도 `FW_THR_MAX`의 59%를 넘지 않았다.
  - **🔴 F-5와 F-6은 같은 사건이다.** 침하를 촉발한 55° 뱅크를 `a_max_g=0.3`(16.7°)이 허가한 적이 없다 — 실제 횡가속이 0.72~1.44g로 **2.4~4.8배 초과**한다. 코너에서 lookahead 목표점이 한 틱에 50.5m 튀고 코스 지령이 20ms 안에 48° 계단이 되기 때문이다.
  - **처방 검증(B2·B3·B4·B5 × 8설정, 28런/유효 24, 코드 변경 0 — PX4 파라미터만):** B4 55° 뱅크·뱅크 정규화 `k=|Δh|/(1−cos φ)` 기준으로 `FW_T_SPDWEIGHT` 1.0→0.5 단독 **−15%**, `FW_T_ALT_TC` 5.0→3.0 단독 **−19%**, 둘 다 **−31%**, `0.0/3.0` **−50%**. → **§8의 "1순위(SPDWEIGHT)가 낫다"는 성립하지 않는다**(둘은 비슷하고 더해진다), **"침하의 76~86%가 배분 오류이니 배분 파라미터로 되찾는다"도 과대평가였다.**
  - **🔴 이 세션의 핵심 발견(§12.6) — 이 처방은 근본해결이 아니라 완화책이다.** 피치 지령은 설정에 따라 16.6°→20.2°로 늘었는데 **수직력 결손은 0.07~0.09로 붙박이**다(9설정 전부). 줄어드는 것은 결손이 아니라 **결손의 지속시간**(3.60→2.61s)이다. 실효 받음각이 20.3→22.7°로 **모델 실속각(19.43°, `model.sdf:688`)을 넘어간다** → §5.3의 "불가피분 ≤1m"는 필요 양력이 실속각 안에서 나온다는 가정이었고 **틀렸다**. **침하를 근본적으로 줄이려면 뱅크를 낮춰야 한다 = F-6.**
  - **속도 안전성:** 전 런 `underspeed_ratio` 0.000, 동적 최저속도 미달 0.00s. 단 **판정선은 정적 `FW_AIRSPD_MIN`(10)이 아니라 뱅크 보정된 `10·√(1/cosφ)`**(55°에서 12.5)여야 한다(`PerformanceModel.cpp:147-152`) — 그 기준으로 보면 여유가 1.35 → **0.39~1.16 m/s로 얇아진다.** PX4에 자동 되돌림은 있다(underspeed 시 speed weight를 2.0으로 강제, `TECS.cpp:387-394`).
  - **🔴 `a_max_g` 손익분기 재평가: 0.41 → 0.47**(권고값) / 0.61(대안 `0.0/3.0`). **0.43은 들어오고 0.50은 여전히 경계 밖** → §8의 "TECS 먼저 하면 0.50까지 열린다"는 **절반만 맞다.**
  - **🔴 방법론 — 다음 세션이 반드시 알아야 할 것 3건:** ①**단일 런 A/B는 성립하지 않는다** — 같은 설정 기준선 4런의 침하가 7.07/8.31/7.49/7.39m(폭 1.24m = 16%). 뱅크 정규화 `k`를 1차 지표로 쓸 것. ②**노드 `range_limit_m` 기본 300m가 B3(354m)·B2(492m) 코너를 OVERRIDE로 끊는다** — 캠페인(2026-07-27)엔 없던 가드(`ca8809d`)라 **캠페인값과 직접 비교 불가**, 전 런 800m로 통일했다. ③**실기체 파라미터를 SITL 기체에 얹는 시도는 실패했다** — `FW_THR_MAX=1.0`에서 TAS 39m/s·뱅크 88.7°·`underspeed` 1.0의 제어 상실(두 런 표에서 제외).
  - **하니스 신설(`47047ed`):** `FW_T_SPDWEIGHT`처럼 **PX4 안**의 값은 `--launch-arg`로 닿지 않아 MAVROS 파라미터 서비스로 넣는 **`--px4-param NAME=VALUE`** 경로를 만들었다(set 뒤 반드시 get으로 되읽어 `meta.json`의 `px4_params`에 기록). ⚠️ 실측 함정 2건이 코드·문서에 박혀 있다 — ①**PX4는 파라미터를 저장한다.** 직전 런의 값이 다음 런까지 따라오므로 "기준선 런에는 아무것도 안 주면 기본값"이 아니다(기준값도 매 런 명시). ②**SITL ulog는 부팅부터 기록되고 `--px4-param`은 그 뒤에 들어간다** — 주입값은 `initial_parameters`가 아니라 `changed_parameters`에 있다(B4_spd05 실측: initial=1.0, changed@32.9s=0.5). `initial`만 보면 "파라미터가 안 먹었다"고 오판한다. ③**파라미터 저장소(`parameters.bson`) 정리는 세션의 마지막 행동이어야 한다**(`7694fb5`).
  - **부수 발견:** `planner_g2_redesign.md` §6(A) "코너에서 감속"은 **오프보드에서 실현 불가**다 — `FixedWingModeManager.cpp:2131`이 `cruising_speed=NAN`으로 하드코딩돼 있다.
...
- **다음(요약) — 2026-07-29 갱신:** ①**R5(경로·고도)**: ~~천이 고도계단(F-9)~~ **→ ✅ 완료·SITL 검증(위 최신 블록)**. 남은 것은 `_cruise_alt` 스칼라화(F-8)·짧은경로 `_FW_LOOKAHEAD`(F-11)·`d_end_thresh` 기본값(F-10, **C1b 회귀가 여기 물렸다 — 위 블록 참조**). **⚠ `d_end_thresh`를 ≈57로 키우면 폐회로가 깨진다** — `R2_closed` FOLLOWING 진입 시 종점까지 12.18m뿐이고 **런간 변동 포함 실제 최악 여유 0.6m**(관측대역 10.60~15.32m). 폐회로에서 반드시 함께 검증할 것. **미규명: MC HOLD 수직가속 상승**(A1 5.98→10.34, C2 6.71→11.73 m/s², B8 불변) ②**R6(선회 품질·정리)**: F-5(선회 중 고도침하 — **A안으로는 안 고쳐진다, 종방향 TECS 문제**)·F-7 헤딩정렬 마진·F-13 미사용 속도프로파일·**F-12 플래너 비동기화**(아래 참조) ③~~**F-17 실기체 플래시**(사용자 승인 대기, 준비 완료)~~ **→ ✅ 완료(2026-07-29, 위 최신 블록 참조)** ④미실측: `param_set` 주입·C9.
  ...
  - ~~**`v_cruise`로 F-12를 못 고친다(실측 확정):** … **F-12의 해법은 비동기화(R6)뿐.**~~
    **→ 🔴 2026-07-29 뒤집힘. F-12 는 근본수정으로 해결됐고 비동기화는 이제 불필요하다(`15f7fa4`).**
    - **성능:** L자 26.1 → **0.098 s**, 폐회로 99.4 → **0.305 s** (오케스트레이터 재현). **경로 불변**(10개 시나리오 최대 변화 1.4e-6 m).
    - **원인은 "NR 반복 3중 낭비"였다**(사용자 가설 적중): ① `clip_attempts` 3회가 **100% 헛돎**(세 attempt 의 `|F|` 가 소수점 15자리까지 동일) ② `max_iter=60` 전량 소진 — **잔차계가 정확히 2개 과결정**(방정식 96 / 미지수 94)이라 `|F|<tol` 이 **원리적으로 도달 불가**이고 정체 판정이 `and` 로 묶여 미발동, 실측 10회에 정지 ③ `_insert_wps_if_infeasible` 이 **수학적 no-op**(중점 삽입 시 Δθ·chord 가 함께 절반 → `k_need` 불변) ④ 야코비안이 대역폭 5 띠행렬인데 조밀 계산(시간의 83%).
    - **⚠ 위 취소선의 "≤16 이 싼 이유는 NR 이 일찍 포기하기 때문"도 틀렸다.** v=16 에서도 NR 은 60회를 다 돈다. 진짜 원인은 **삽입 발동 문턱** `0.9·a_max/v² = 2Δθ/chord → v = 16.93` 이다. 같은 오기가 `fc_ros/fc_ros/params/fc_ros_params.yaml` 의 `v_cruise` 주석에도 남아 있다(정정 대상).
    - **🔴 더 나쁜 사실:** `v_cruise=18` 에서 이 플래너가 내는 경로는 **폴리라인과 구별 불가**다(최대 이탈 0.15 m, 코너 헤딩 꺾임 **85.94°**, `|κ|max` 가 한계의 **161배**). 19.6초를 계산해 `StraightLinePlanner` 와 같은 것을 낸다. 삽입점이 **폴리라인 위에** 놓이기 때문.
  - **경로 품질 재설계("안 C") 1단계 완료 — 프로토타입 상태로 동결**(`afce94d`·`234de07`, 설계문서 `docs/planner_g2_redesign.md`). 새 모듈 `vtol_sim/path_planning/g2_spiral_planner.py` 가 **기존 플래너와 나란히** 있고 `run_scenario.py --planner g2spiral` 로 수동 선택된다. 실측(오케스트레이터 재현): `|κ|max/κmax` A3_L 161.48→**1.42** / R2_closed 161.51→**1.25** / B5 164.01→**1.28**(Pontryagin **이론하한 1.26** 에 1.6% 근접) / B2_gentle **0.52**(κmax 이하), 헤딩 꺾임 85.94°→**0.55~0.63°**, WP 오차 **3e-07 m(affine 보정 없음)**, 계산시간 폐회로 1.374→**0.102 s**. 짧은 경로는 `FeasibilityReport` 로 **명시 출력**(`S_L30` `v_feasible 5.85 m/s`, `S_line40` 감속여유 −40 m). 전장은 **8~12.5%** 증가(세션이 보고한 20~30% 는 과장, 재현치로 정정).
    - **`|κ| ≤ 1.0` 은 달성 불가다 — 플래너 탓이 아니다.** 곡률 불연속까지 허용한 bang-bang 이론하한이 이미 A3_L 1.13 / B5 1.26 배다. "18 m/s 로 200m 다리 90° 코너를 WP 를 밟으며 돈다"가 기하학적으로 불가능하다.
    - **🔴 2단계(교체 + SITL 전면 재검증)는 사용자 결정으로 보류**(2026-07-29). `a_max_g` 상향(0.3→0.43/0.50)도 **F-5 결과를 보고 판단**하기로 했다 — 순서는 **TECS 먼저, `a_max_g` 나중**. **→ 2026-07-29 F-5 결과 나옴(위 F-5 블록): 손익분기 0.41→0.47이라 0.43은 들어오고 0.50은 여전히 경계 밖.** 즉 TECS 처방을 넣어도 0.50은 열리지 않는다. `a_max_g`는 **0.43까지만** 사용자 결정 대상이다.
  - **회귀 시나리오 필수요건에 「비-정북 *천이*」 추가** — 캠페인 24런 중 비-정북 천이는 C2·B8 2건뿐이었다(A3/B4/B5는 코너가 있어도 첫 레그가 정북). §7-1의 "비-정북 *레그*"만으로는 F-17을 못 잡았다.
- **참조:** **`logs/2026-07-27_sitl_vtol_campaign/campaign_report.md`(종합)** · `sitl_vtol_fw_offboard_rootcause.md`(근본원인 전문) · `sitl_vtol_campaign.md` · `sitl_vtol_static_audit.md` · `logs/2026-07-27_sitl_vtol_campaign/PX4_BUILDS.md`(런↔빌드 대응) · `tools/sitl/README.md`(하니스) · `sitl_verification_log.md` · **`fc_f5_turn_altitude_loss.md`(F-5 원인·처방 전문, §12가 SITL 실측표)**

### ✈ vtol-실기체 — ▶ 활성 (**2026-07-30 2차예선 전 마지막 비행 7건 완료. 근본원인 확정: 추력 부족 T/W 1.44 + CG 전방편향. 다음은 비행이 아니라 지상 조치**)
...
- **마지막:** **(2026-07-30, 실비행 7건 + manual 회수·분석 — 2차예선 전 마지막 비행. 근본원인 확정)** 수동 1건(`log_266`) + offboard 7건(`log_268`/`269`/`274`/`276`/`279`/`280`/`282`) 회수·`pyulog` 정밀분석. ulog↔rosbag 매칭은 metadata UTC 창 vs ulog 내부 개시시각으로 개별 수행(`collect_new_logs.py`는 미회수 197건 전량 스윕이라 미사용). 목표는 전 비행 공통 `transition_alt=20.0m`인데 **7건 중 20m 도달은 f02 하나뿐(20.07m)**, 나머지는 5.9~15.7m에서 멈췄고 **f07은 14.1m에서 무지령 강하 → 피크 8.2G로 접지(추락)**. **한 줄 결론: 기체는 고장난 게 아니라 추력이 모자란다.** ①**H19 신설(상위원인)** — `hover_thrust_estimate=0.693` → **T/W ≈ 1.44**. 호버에 이미 추력의 69%를 쓰고, 상승률이 지령 1.5m/s 대비 실측 0.44~0.86m/s에 그친다. f05/f07은 `AUTO_TAKEOFF` 상태에서 무지령 강하했고, **만충 팩이던 f03조차 0.83m/s** — 배터리를 고쳐도 남는 제약이다. ②**H8 확정·정량화(H13을 대체하는 정답)** — f02 안정호버 61초(자세 거의 수평 = 자세지령이 아닌 상시 트림)에서 앞(m0+m2)/2=0.795 vs 뒤(m1+m3)/2=0.639 → **추력비 1.55배**, 우 0.765 vs 좌 0.669 → 1.31배. 모멘트 평형 역산 시 기하중심 대비 **앞으로 세로 축간거리의 약 11%, 우로 가로 축간거리의 약 7%**. 대가는 **m0(전우)가 0.836에서 천장에 먼저 닿을 때 m1(후좌)은 0.584로 여유 0.42를 사장**하는 것 — 기체 전체의 상승능력이 가장 혹사당하는 모터 하나에 묶여 있다. **조치는 비용 0**: 저울·수평계로 CG 실측 후 배터리·페이로드를 뒤·왼쪽으로 이동하면 임계모터 여유가 +0.16 늘어난다. ③**H13 기각 확정** — CW대각(m0,m1)=0.710 vs CCW대각(m2,m3)=0.724, 차 **−0.014(추력비 1.00배 = 차이 없음)**. 07-21에 "CCW쌍이 더 일한다", 07-25에 "CW쌍이 더 일한다"로 두 번 뒤집혔던 것은 **대각선 축이 애초에 실재하지 않아 표본마다 부호가 흔들린 것**이었다. **④천이 실패(f02)의 원인은 피토 영점이다 — 푸셔는 정상 복구됐다.** 07-25 정상 기준(전류 +8.5~15.9A, body ax +1.0~1.9)과 같은 창 정의로 07-30 f02는 **전류 +10.85A, body ax +1.46, 대지속도 11.19m/s로 정상 범주**(07-28/29의 무구동 상태는 해소). 그런데 **지상 정지 상태에서 차압 원시값이 −42.6Pa인데 저장된 영점이 `SENS_DPRES_OFF=−21.78`로 약 21Pa 어긋나 있어**, 기체가 멈춰 있는데 CAS가 −5.9m/s로 찍힌다(7비행 전부 동일). PX4는 CAS를 **최대 8.8m/s(t=33~38 평균 5.3)로 저평가** → `VT_ARSP_TRANS=12.0` 미달 → 천이 시작(t=27.19) **11.1초 뒤 Quad-chute**(t=38.30, `fixed_wing_system_failure=1` 래치), `vehicle_vtol_state`는 3(MC)→1(TRANS_TO_FW)→3(MC)으로 **FW(4)에 한 번도 닿지 못했다**. **우리 코드 탓이 아니다** — `VTOL_TRANSITION(p1=4)`이 t=27.19에 1회 나갔을 뿐 역천이 명령이 없고 `offboard_node` 로그도 조종사 인계(t=106)까지 조용하다. **🔴 처방 — 2026-07-31 두 차례 정정됐다. 옛 '두 줄이면 된다'는 폐기.** ⓐ**"영점 또는 `VT_ARSP_TRANS` 둘 중 하나만 고쳤어도 천이 성공"은 과장이었다** — 온도보정한 실제 대기속도는 **10~11m/s**(대지속도 11.2 − EKF바람 1.1과 정합)이고 영점을 고쳐도 PX4가 봤을 값은 평균 8.0/최대 10.1m/s라 **12엔 미달, 10이어도 간신히 걸친다.** **→ 천이 실패는 피토 단독이 아니라 H19(추력부족)의 또 다른 얼굴이다** — 푸셔 100%를 10초 써도 10~11m/s에서 멈춘 것은 리프트 로터가 계속 0.4~0.6으로 기체를 들고 있어야 해서다. ⚠️🔴 **중량 실측 전 `VT_ARSP_TRANS` 를 내리지 말 것** — `FW_AIRSPD_STALL=7`은 설계중량 기준이고 실속속도는 **중량의 제곱근에 비례**한다. 호버 스로틀 0.69가 말해주듯 이 기체는 설계보다 무거우므로 지금 천이속도를 낮추면 **오히려 실속 위험이 커진다.** ⓑ**영점 이동의 원인이 온도라는 것도 미입증이다** — 초판은 회귀 `raw=25.65−1.906·T`(r²=0.956)로 '온도 확정'이라 했으나, 그 표본은 저온군(27~30°C, 07-25~29)과 고온군(35.6~37.6°C, **전부 07-30**) 두 덩어리라 **사실상 2점 추정이고 온도와 날짜가 완전 교락**돼 있으며 군내 검정 5건이 −4.88~+2.40으로 흩어진다(검정력 0). '캘 온도 25.4°C가 차량 실내와 일치'도 순환이었다(기울기를 −0.6~−4.0 어디로 잡아도 18~27°C가 나온다). **확실한 것은 영점이 07-29→07-30에 15.7Pa(날짜간 산포의 13배) 이동해 정지 CAS −5.9m/s를 만든 것뿐**이고, 원인은 **온도/결로/배관 셋이 경합** 중이다. **→ 이 스레드는 별도 오케스트레이션 세션이 인수했다**(`docs/fc_pitot_temp_drift.md` — 실비행 ulog 52건·영점표본 120개로 재회귀, 개선 추정치 **−1.387 Pa/°C** [95%CI −1.55…−1.23], Phase 0 결로판정 완료). 실험 절차서는 `docs/fc_pitot_thermal_experiment.md`(사전등록), 벤치 로거는 `tools/pitot_bench/`. **처방 확정은 그 세션의 결과를 기다린다.** ⚠️ **정확한 quad-chute 사유코드는 디코드 못 했다**(펌웨어 `events.json` 메타데이터가 로컬에 없음) — 소거법으로 `VT_FW_QC_*`/`VT_QC_ALT_LOSS`는 전부 0(비활성), `VT_FW_MIN_ALT=10m`은 당시 17.5m라 미해당, 천이 중 고도손실 1.07m(`VT_QC_T_ALT_LOSS=20m`) → **"천이 미완료 타임아웃" 계열만 남는다.** ⚠️ 배터리 내부저항 **84~107mΩ(셀당 25mΩ)** 실측 — 두 팩 모두 동급이라 **H1/H2 vs H3는 여전히 로그로 못 가른다. 부하 중 단자전압 직접측정이 유일한 갈림길.** 상세: `logs/2026-07-30_review.md`(해석 정본) · 비행별 `logs/2026-07-30_flight0N/notes.md` · 가설 테이블 `docs/mc_hw_open_hypotheses.md`(H8/H13/H15/H19 갱신됨).
- **그 전 (2026-07-29, 실비행 11건 로그 회수 + 푸셔 결선·추력 방향 감사)** `record_flight.sh` 폴더 11건(rosbag+launch.log) + FC ulog 22건 회수 — flight01~04 = mc, flight05~07 = vtol 정북 80m, flight08~11 = vtol 정남 80m. **flight06이 이 날 유일하게 실제로 천이를 시도한 비행**이자 사용자가 "정북을 입력했는데 남쪽으로 갔다"고 보고한 그 비행이다. **원인은 항법이 아니다** — 발행 setpoint 정확(ENU E=0, N=+80, U=24, yaw=정북 0°), EKF yaw vs GPS 진행방위가 t=40/48s에 +1.3°/+0.9°(= **자기계 180° 오류 아님**). t=36.0s에 그 차이가 −179.4° = **코를 북에 둔 채 뒤로 밀렸다.** 8초에 고도 13.4m 손실 + 남쪽 12.5m 이탈 → 조종사 POSCTL 인계. **푸셔 감사(사용자 의문: "결선이 뒤집혀 추력이 후방으로 나가는 것 아닌가")** — 전체 103개 ulog 중 푸셔가 명령된 7건을 전수 대조. **판정: 결선 방향은 옳다. 문제는 방향이 아니라 푸셔가 구동되지 않는 것이다.** 신호경로 정상(`CA_ROTOR4_AX=+1.0`, `PWM_AUX_FUNC5=105`, `PWM_AUX_REV=0`, flight06에서 AUX5가 1900us를 5.5초 연속 수신, 반전 파라미터 없음). 07-25 `flight17`/`log_141`/`log_142`는 같은 지령에 전류 +8.5~15.9A·body ax +1.0~+1.9였고 `log_142`는 0→+11.95m/s로 전방 90.8m 이동(피토 dP −40→+42Pa) — **결선·프로펠러 방향이 원래 옳았다는 실측**. 반면 07-28 flight02는 안정 호버에서 푸셔 100%를 9.8초 유지했는데 동체전방속도가 −0.58~+0.36m/s를 벗어나지 않고 적분 전방이동 −1.0m — **역추력이면 강한 후진이 있어야 하는데 전방도 후방도 아닌 0이다.** 전류증가도 07-28 −0.3A / 07-29 +0.7A(역회전 프로펠러도 전류는 끈다) → **무구동 확정.** **남쪽 이탈의 실제 원인:** 이 기체는 상시 −4~−6.5° 기수하향 호버 트림을 갖고(07-25/28/29 전 로그에서 ax = 9.81·sin(pitch)로 일치), 천이 진입 시 `VT_PITCH_MIN=0`으로 그 트림이 풀리며 균형중이던 **약 0.9 m/s² 후방력이 남았다** — 검산 −0.41 vs EKF 실측 −0.42 m/s²로 폐합. 07-25 flight17은 같은 −5.98° 트림에서도 **살아있는 푸셔로 이를 압도해 전진**했다. **부수 정정:** flight06 notes.md의 "t=31.5s 얼로케이터 이상"은 **킬스위치였다**(`actuator_armed.kill` 0→1→0 t=30.96~31.44, 5개 출력 동시 1000us, battery I=0.00A/V=14.91 무부하 복귀 — AGL 20m에서 0.48초 전 모터 정지). **고장구간: 마지막 정상 07-25 11:22 → 최초 이상 07-28 10:29.** 사이에 PX4 재플래시가 있었으나 푸셔 관련 파라미터는 diff 무변경(바뀐 39개는 IMU/mag 캘리브레이션·baro·RC 매핑뿐) → **AUX5 핀 이후 물리경로 문제.** 점검순서는 3상 순서가 아니라 **ESC 전원 → 상 도통 → 시그널선** 순. 전문: `docs/fc_pusher_thrust_audit_2026-07-29.md`.
- **그 전 (2026-07-28, 실기체 첫 MC→FW 천이 시도 — flight01/02, 천이 실패)** 원인 판정이 세 번 정정됐다: "독립된 하드웨어 결함 2건" → **pusher 추력 부재 하나로 수렴(핏토관 판정 철회)** → 독립 재현검증으로 전류 근거 1건 교체·프롭 뒤집힘 배제. PX4 `logged_messages` 누락을 시정해 **Quad-chute·자기계 결함·배터리 emergency** 3종을 뒤늦게 확인. **배터리 Emergency는 오탐**으로 규명(전 비행 공통) → **사용자 결정으로 종결(무시하고 운용)**, `COM_LOW_BAT_ACT` 함정을 문서에 명시. **자기계는 미해결** — 재캘리 적용은 확인됐으나 원인이 **헤딩 의존 오차**이고 지상 진단에서 |B|의 헤딩 의존이 검출됐다(야외 4방위 측정 필요). 천이 실패 원인 가설 중 "천이 중 위치 유도" 전제는 **PX4 소스로 반증**. 인프라: flight01/02의 **U+00A0(비분리 공백) 사고** 재발 방지용 **launch 인자·파라미터 위생 검사 3겹** 신설 + **비행 전 물리문제 체크리스트** 신설(`docs/preflight_physical_checklist.md`, 이후 `fc_bridge` PYTHONPATH 항목 추가 — md5로 못 잡는 사각지대).
- **그 전 (2026-07-24, 비행 전 코드 준비상태 점검 — CV 미사용 예정 비행, `planner`/`vehicle_type` 파라미터 동작 확인 + 크래시 버그 1건 발견·수정)** 사용자 질의("MC 테스트기체 코드수정이 있었는데 VTOL 비행해도 괜찮나") 계기로 `offboard_node.py`/`fc_bridge/planning/planner_runner.py`/`eta3clothoid_v3_1_planner.py`/`straight_line_planner.py` 코드 직접 확인. **결론: MC용 코드수정(yaw setpoint 수정 등, 07-21)은 경로계획 모듈과 무관 — 영향 없음.** 확인된 사실: ①`planner:="auto"`는 `vehicle_type=mc`→`straight`(NR/최적화 없는 단순 3D 직선, 완전중복 WP만 병합, 그 외 거부 없음), `vehicle_type=vtol`(기본값)→`eta3`로 자동분기(`resolve_planner_name()`). ②eta3는 짧은 경로/급선회에도 "거부"하지 않음 — WP가 2개뿐이면 NR 자체를 건너뛰고(N≤2 특수케이스), 3개 이상이면 NR 잔차가 커도 affine 보정으로 WP 통과를 강제하고 콘솔 WARNING만 출력(예외 없음). 실제 `raise`는 "병합 후 distinct WP가 2개 미만"(사실상 한 점) 경우뿐. ③명시적으로 `planner:="straight"`를 VTOL 기체에 줘도 됨(항상 auto보다 우선) — 단 곡률 불연속을 그대로 노출하므로 FW/천이구간엔 부적합, MC구간 검증용으로만. **✗ 그러나 점검 중 별개의 실제 크래시 버그 발견:** eta3 NR 해석 경로(WP 3개 이상)가 `np.trapz`를 호출하는데, 이 개발머신 numpy(2.5.1)에서 `np.trapz`가 완전히 제거되어 있어 `AttributeError`로 즉시 크래시함 — "경로가 짧아서 거부"가 아니라 numpy 버전 비호환 크래시. 리포 자체 회귀테스트(`vtol_sim/tests/test_eta3_v3_degenerate_wp.py::test_normal_wps_unaffected`)도 이 크래시로 실패 상태였음(`fc_bridge/tests/`만 pytest 대상이라 이 결함이 CI로 안 잡혀왔던 것으로 보임). **`_trapz()` 로컬 사다리꼴적분 함수로 교체해 수정, `fc_bridge/tests`(65)+`vtol_sim/tests/test_eta3_v3_degenerate_wp.py`(3) 전부 통과 확인, `dev--vision-computing-module`에 커밋·push 완료(`0785777`).** **⚠ 미확인 — 이 비행 전 반드시 확인 필요: RPi5 companion computer의 fc_bridge venv numpy 버전.** 그쪽 numpy가 2.x 계열(trapz 제거 버전)이면 실비행 중에도 동일 크래시 재현 가능 — 이번 커밋이 이미 반영됐는지, 혹은 numpy가 애초에 구버전(trapz 존재)인지 먼저 확인할 것. 관례대로 이 수정은 SITL 회귀검증 전 실비행 투입 비권장이나, WP를 2개로만 구성(예: 천이점→착륙점 단일 leg)하면 N≤2 특수케이스로 이 NR 경로 자체를 안 타므로 당장은 우회 가능. 그 전 2026-07-09 — VTOL 테스트기체 조립 완료했으나 기체 결함으로 비행 불가했던 이력, 이후 수리 경과는 사용자 구두 확인 기준(이 문서에 별도 수리완료 로그는 없음). **PX4 파라미터로 mc/vtol 물리 형상 구분 불가**(공통 상태 참조) — 실기체 세션 시작 전 어느 기체가 붙어 있는지 이 문서로 먼저 확인할 것.
