# 자동분석 — /home/suri/suridoksuri/logs/2026-07-26_manual

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_199_2026-07-25-11-10-42.ulg` (51.22s)
...
### 고도
- home_position AMSL=20.81m, EKF ref_alt AMSL=22.0m
- AGL 최대 8.34m (t=17.27s), 최소 -0.01m (t=0.77s), 종료시 1.16m
...
### 자세
- roll 최대|값|=13.02° (t=9.97s)
- pitch 최대|값|=-10.53° (t=10.27s)
- roll 목표(setpoint) 최대|값|=14.04° (t=9.76s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=5.07s (-10.1deg/s)
- pitch rate 이상조짐 후보: t=4.93s (13.2deg/s)
- yaw rate 이상조짐 후보: t=22.99s (8.4deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.72 max=1.0 min=0.002
- 1:후좌: mean=0.479 max=0.698 min=0.0
- 2:전좌: mean=0.599 max=0.839 min=0.002
- 3:후우: mean=0.637 max=0.873 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.73V, 부하중 최저 13.24V, 전류 최대 26.3A
- remaining: 0.96 → 0.6

### 실패감지기
- imbalanced_prop_metric_max_abs: -9.925
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=3.67s
