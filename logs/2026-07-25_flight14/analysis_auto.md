# 자동분석 — logs/2026-07-25_flight14

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_190_2026-07-25-10-48-48.ulg` (36.97s)
...
### 고도
- home_position AMSL=23.07m, EKF ref_alt AMSL=22.0m
- AGL 최대 5.11m (t=22.47s), 최소 -0.08m (t=4.37s), 종료시 -0.07m
...
| 1.7 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 10.93 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 14.01 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 23.7 | 18:AUTO_LAND | 18:AUTO_LAND | 0 | ARMED | INTENTIONAL_CHANGE |
| 35.98 | 14:OFFBOARD | 14:OFFBOARD | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=24.28° (t=21.07s)
- pitch 최대|값|=-27.29° (t=15.73s)
- roll 목표(setpoint) 최대|값|=28.46° (t=20.77s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.94s (-12.5deg/s)
- pitch rate 이상조짐 후보: t=5.08s (8.2deg/s)
- yaw rate 이상조짐 후보: t=5.31s (-8.6deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.716 max=1.0 min=0.002
- 1:후좌: mean=0.577 max=1.0 min=0.0
- 2:전좌: mean=0.561 max=0.983 min=0.002
- 3:후우: mean=0.592 max=0.97 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.3V, 부하중 최저 11.52V, 전류 최대 34.4A
- remaining: 0.44 → 0.2

### 실패감지기
- imbalanced_prop_metric_max_abs: -8.633
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=2.67s
