...
**이 세션이 띄운 백그라운드 트랙 3개는 전부 종료됐다**(§4). 재현 검증도 끝났다. 그러니:

1. **`git fetch origin && git log --oneline -15`** 로 브랜치 tip을 확인하고, `pytest vision/tests/`가
   **886 passed**인지 본다(다른 세션이 그 뒤로 올렸을 수 있다).
2. **§3-4의 FC 변동 확인 명령**을 돌린다. 바뀌었으면 §3-4의 "볼 것" 목록대로 계약 영향을 판단한다.
3. **§5-1의 열린 결정**을 사용자에게 확인한다 — 특히 **해상도 vs 발행 주파수**(§3-5)가 실기체 성능에 직결된다.
4. 그 다음 §8 백로그로 들어간다. **폐루프를 닫는 마지막 칸은 FC 도메인**(`OffboardNode` 정밀착륙 서브상태)이라
   vision 세션이 할 수 없다 — 사용자에게 FC 트랙 착수를 제안하는 것까지가 범위다.
