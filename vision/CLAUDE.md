...
| fusion `TemporalFusion` | 임계 미달 시 confirmed/meta 없음·정확히 `min_frames`번째 프레임에 승격·`min_frames`/`iou_threshold` 설정값대로 동작(하드코딩 아님)·확정 카운트 단조증가 · **흔들림 억제 핵심**: 한 프레임 걸러 나타나는 검출은 20프레임을 줘도 미승격, 1프레임 스파이크는 잔여 후보 없이 소멸 · 타깃 소실 시 감쇠→재확정에 프레임 재소요 · IoU 정확값·임계가 병합/분리를 가름·떨어진 두 타깃 독립 누적 + 증거 많은 쪽 확정 · 빈 입력 안전·선언필드 계약·결정론. **파괴검증 12종(F-M1~M12)으로 red 확인**. ⚠️ **알려진 결함 고정(characterization)** — `_decay()`가 방금 매칭된 후보의 count도 깎아 **프레임당 검출 1개짜리 타깃은 영원히 승격되지 않는다**(docstring과 어긋남). 문서화된 계약은 `xfail(strict=True)`로 박아둠 → 고치면 XPASS로 뒤집혀 알림. 나머지 테스트는 "한 프레임에 겹치는 검출 2개 이상"에서만 관측되는 현재 산술(프레임당 순증 +1)을 고정하므로 decay 수정 시 함께 갱신할 것 | ✅ test_fusion (결함 1건 미해결, 좌측 참조) |
