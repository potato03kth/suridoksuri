...
| **작업 G — 로그 자동수집 체계** | ✅ **완료** (코드 `b580953`, 검증 V2 `f171fb4`) | [아래](#작업-g--비행-로그-자동수집분석-체계-인프라) |
...
**업로드 방침 (2026-07-06 결정 → 2026-07-18 번복):** ~~GitHub 업로드 안 함~~. 여러 기기(RPi·개발컴·노트북)에서 로그를 git만으로 공유·접속하기 위해 **일반 git 커밋으로 전환**(사용자 결정, LFS 아님) — `logs/`를 `.gitignore`에서 제외하고 rosbag(`.db3`)·ulog(`.ulg`)를 그대로 커밋한다. **트레이드오프를 인지한 채로 내린 결정:** git 이력이 로그 바이너리만큼 영구히 커지며(되돌려도 이력 크기는 안 줄어듦), 저장소 clone/fetch가 계속 느려진다. 대안(Git LFS, 로그 제외하고 SSH/scp 공유만 유지)은 검토했으나 기각. 공유·웹 분석이 필요한 ulog는 선택적으로 PX4 Flight Review(logs.px4.io)에도 업로드 가능.
