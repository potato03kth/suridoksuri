...
> # ⚠️ 이 문서는 대체되었다 (2026-07-28)
> **정본은 `docs/vision_orchestration_handoff.md` 다.** 다음 세션은 그 문서로 진입하라.
>
> 아래 내용은 2026-07-25 시점 기준이며 상당수가 **이미 소화되었거나 사실과 다르다:**
> - §1 "먼저 물어라" 2번(다음 우선순위) — 결정됨. §2a(FC 연동) 착수·완료(vision 절반).
> - §2c(테스트 커버리지 갭) — `fusion`/`tracker`/`registry`/`image_loader`/`video_reader` 완료.
> - §2d 색 캘리브레이터 마진 기본값 — **`hue=6` 으로 확정됨**(커밋 `1678539`). 여기 적힌 "기본값이 0"은 옛 사실.
> - §3 환경 함정 — 신규 항목이 추가되어 **`vision_orchestration_handoff.md` §7 로 이관**됐다.
>
> 여전히 유효한 것: §1 1번(H.264 육안 확인, 아직 미실시) · §4(절대 하지 말 것) · §5(참조).
