...
### 고도
- home_position AMSL=20.35m, EKF ref_alt AMSL=26.48m
- AGL 최대 26.84m (t=67.65s), 최소 -1.26m (t=131.35s), 종료시 -1.26m

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.42 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
...
### 자세
- roll 최대|값|=32.96° (t=56.81s)
- pitch 최대|값|=33.45° (t=32.66s)
- roll 목표(setpoint) 최대|값|=-31.36° (t=32.11s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=5.6s (-13.6deg/s)
- pitch rate 이상조짐 후보: t=5.68s (10.0deg/s)
- yaw rate 이상조짐 후보: t=13.72s (8.7deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.534 max=0.916 min=0.002
- 1:후좌: mean=0.572 max=0.984 min=0.0
- 2:전좌: mean=0.511 max=0.912 min=0.0
- 3:후우: mean=0.61 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.91V, 부하중 최저 12.2V, 전류 최대 40.5A
- remaining: 1.0 → 0.41

### 실패감지기
- imbalanced_prop_metric_max_abs: -13.012
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=4.66s
