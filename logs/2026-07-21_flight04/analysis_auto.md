# 자동분석 — logs/2026-07-21_flight04

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_34_2026-07-21-11-12-30.ulg` (33.96s)
...
### 고도
- home_position AMSL=24.45m, EKF ref_alt AMSL=25.33m
- AGL 최대 4.4m (t=20.87s), 최소 -1.79m (t=31.66s), 종료시 -1.77m
- notes.md 목표 transition_alt=4.0m → **달성**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.5 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 1.38 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 7.24 | 4:AUTO_LOITER | 17:AUTO_TAKEOFF | 1 | ARMED | FAILSAFE_FORCED |
| 12.04 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 15.99 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 18.18 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 20.38 | 18:AUTO_LAND | 18:AUTO_LAND | 0 | ARMED | INTENTIONAL_CHANGE |
| 32.97 | 14:OFFBOARD | 14:OFFBOARD | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=13.33° (t=4.86s)
- pitch 최대|값|=-6.21° (t=19.91s)
- roll 목표(setpoint) 최대|값|=14.04° (t=19.47s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=3.69s (13.2deg/s)
- pitch rate 이상조짐 후보: t=4.55s (9.3deg/s)
- yaw rate 이상조짐 후보: t=4.33s (11.0deg/s)
> 휴리스틱 후보일 뿐 — 잡음(±5deg/s 이내)에서 15deg/s 이상으로 튄 첫 시점. 이 근처를 §5 얼로케이터 포화 시점·§9 이함 시점과 대조해 진짜 트리거인지 육안 확인할 것.

### 얼로케이터 포화 (축별)
- roll(x): t=1.17s부터 포화 시작 (unallocated=0.055)
- pitch(y): t=30.97s부터 포화 시작 (unallocated=-0.1358)
- yaw(z): t=12.76s부터 포화 시작 (unallocated=-0.0721)
- 통합 achieved 플래그 True 비율: 0.667
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.428 max=1.0 min=0.0
- 1:후좌: mean=0.472 max=1.0 min=0.0
- 2:전좌: mean=0.63 max=1.0 min=0.0
- 3:후우: mean=0.737 max=1.0 min=0.0

### 배터리
- 휴지전압 11.91V, 부하중 최저 8.96V, 전류 최대 38.5A
- remaining: 0.89 → 0.28

### 실패감지기
- imbalanced_prop_metric_max_abs: -22.283
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=2.32s
