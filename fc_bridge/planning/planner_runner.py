...
def planner_eta_s(planner_name: str, n_waypoints: int,
    ...
    **코너당 75s → 90s 상향 (2026-07-28, `v_cruise` 정식값 18.0 확정 세션).**
    ...
    L자 16→0.46s / 17→23.4s, 폐회로 16→27.4s / 17→101.8s. ≥17 에서 비싼 진짜
    이유는 `_insert_wps_if_infeasible` 의 발동 문턱(0.9·κmax = 2Δθ/chord)이
    **v_cruise≈16.93 에서 넘어가** WP 수가 16배로 불어나기 때문이다
    (2026-07-29 규명, 아래 참조).

    ─────────────────────────────────────────────────────────────────────
    🔻 **2026-07-29: 플래너 NR 낭비 제거로 이 어림은 이제 200배 과대예고다.**

    `eta3clothoid_v3_1_planner._solve_g2_nr` 의 계산량 대부분이 **결과에
    영향을 주지 않는 반복**이었음을 계측으로 확인하고 제거했다(§F-12):

        L자 3WP  v_cruise=18 : 19.59 s → 0.126 s  (155배)
        폐회로 5WP v_cruise=18 : 110.7 s → 0.404 s  (274배)
        10개 시나리오 전체에서 **경로 최대 변화 1.4e-6 m** (사실상 동일 출력)

    즉 F-12 의 "블로킹" 자체가 **1초 미만**으로 줄어 현장에서 "죽은 줄 알고
    재기동" 하는 상황이 성립하지 않는다. 이 함수는 **의도적으로 그대로
    둔다** — 위에 적힌 대로 예고는 실측보다 커야 안전하고, RPi5 실기체에서의
    실측이 아직 없기 때문이다(개발 노트북 대비 몇 배 느릴 수 있다).
    RPi5 실측이 잡히면 `per_corner_s` 를 그 값의 3~5배로 내리면 된다.
    """
