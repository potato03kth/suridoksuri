...
### 고도
- home_position AMSL=365.81m, EKF ref_alt AMSL=366.66m
- AGL 최대 4.81m (t=14.66s), 최소 -0.14m (t=21.66s), 종료시 -0.1m

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.5 | 0:MANUAL | 0:MANUAL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 1.0 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 9.14 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 11.9 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 14.5 | 18:AUTO_LAND | 18:AUTO_LAND | 0 | ARMED | INTENTIONAL_CHANGE |
| 26.17 | 14:OFFBOARD | 14:OFFBOARD | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=22.27° (t=12.62s)
- pitch 최대|값|=11.97° (t=12.62s)
- roll 목표(setpoint) 최대|값|=21.52° (t=12.31s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=3.43s (-11.5deg/s)
- pitch rate 이상조짐 후보: t=4.11s (-14.6deg/s)
- yaw rate 이상조짐 후보: t=7.07s (8.3deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.54 max=0.935 min=0.0
- 1:후좌: mean=0.542 max=0.996 min=0.0
- 2:전좌: mean=0.449 max=0.987 min=0.0
- 3:후우: mean=0.537 max=0.99 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.52V, 부하중 최저 12.17V, 전류 최대 33.6A
- remaining: 0.59 → 0.43

### 실패감지기
- imbalanced_prop_metric_max_abs: -7.039
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=1.96s
