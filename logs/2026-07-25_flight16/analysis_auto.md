# 자동분석 — logs/2026-07-25_flight16

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_196_2026-07-25-11-01-00.ulg` (40.24s)
...
### 고도
- home_position AMSL=22.12m, EKF ref_alt AMSL=22.0m
- AGL 최대 5.15m (t=17.36s), 최소 0.0m (t=0.06s), 종료시 0.66m
- notes.md 목표 transition_alt=5.0m → **달성**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.38 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.39 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 10.14 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 12.99 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 26.79 | 18:AUTO_LAND | 18:AUTO_LAND | 0 | ARMED | INTENTIONAL_CHANGE |
| 39.24 | 14:OFFBOARD | 14:OFFBOARD | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=-20.25° (t=17.46s)
- pitch 최대|값|=-28.37° (t=14.57s)
- roll 목표(setpoint) 최대|값|=-22.85° (t=18.11s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.16s (-11.2deg/s)
- pitch rate 이상조짐 후보: t=4.44s (10.0deg/s)
- yaw rate 이상조짐 후보: t=5.17s (8.6deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.738 max=1.0 min=0.002
- 1:후좌: mean=0.619 max=1.0 min=0.0
- 2:전좌: mean=0.646 max=1.0 min=0.0
- 3:후우: mean=0.674 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 14.87V, 부하중 최저 9.85V, 전류 최대 32.8A
- remaining: 0.29 → 0.0

### 실패감지기
- imbalanced_prop_metric_max_abs: -7.406
