...
### 고도
- home_position AMSL=365.23m, EKF ref_alt AMSL=366.66m
- AGL 최대 5.41m (t=15.27s), 최소 -0.03m (t=3.97s), 종료시 0.47m

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.5 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.81 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 9.01 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 11.61 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 15.51 | 18:AUTO_LAND | 18:AUTO_LAND | 0 | ARMED | INTENTIONAL_CHANGE |
| 27.91 | 14:OFFBOARD | 14:OFFBOARD | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=25.24° (t=12.92s)
- pitch 최대|값|=11.42° (t=12.87s)
- roll 목표(setpoint) 최대|값|=27.97° (t=12.66s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=2.57s (-10.2deg/s)
- pitch rate 이상조짐 후보: t=3.97s (10.2deg/s)
- yaw rate 이상조짐 후보: t=2.77s (-9.4deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.594 max=0.914 min=0.002
- 1:후좌: mean=0.579 max=1.0 min=0.0
- 2:전좌: mean=0.464 max=0.88 min=0.002
- 3:후우: mean=0.561 max=0.965 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.5V, 부하중 최저 11.99V, 전류 최대 34.5A
- remaining: 0.52 → 0.36

### 실패감지기
- imbalanced_prop_metric_max_abs: -7.85
