...
- **마지막:** **(2026-07-20, flight01 제어상실 사고 분석+수정)** 사용자 보고: 직전 실비행(`logs/2026-07-20_flight01/`, 다른 worktree에만 존재·git 미커밋) 중 상승 직후 기체가 순간 제어를 잃고 롤/피치/요가 급변, 조종사가 RC로 수동 회수. ulog(`log_18`) 분석 결과 **STREAMING(321행)/`_step_following()`(775행)이 FW 전용 70m lookahead(flower-pattern 회피용 pursuit 유도)를 MC에도 그대로 적용**해, 짧은 경로(~12m)에서 lookahead가 항상 경로 끝점(WP1)으로 클램프 → OFFBOARD 진입 첫 순간 **실제 위치(클라이밍 중 수평드리프트+고도오버슈트로 (-4.4,1.2,7.3m))와 무관한 절대좌표 `(0,0,4.0)`·yaw 90°로 순간점프 발행** → PX4가 급격한 자세보정을 시도해 roll -16°/pitch -30.8°/yaw rate 최대186°/s. **수정:** MC는 PX4 OFFBOARD 속도 세트포인트를 정상 추종(FW와 달리)한다는 점에 근거해 STREAMING은 0속도 스트리밍, FOLLOWING은 `L1Guidance.ned_velocity_cmd()`(기존 존재하던 속도기반 인터페이스)로 전환 — FW 경로는 미변경. `fc_bridge`(rclpy 비의존) 순수함수로 사고 시점 실측좌표 대입해 수정 전(4.54m 즉시 절대변위 요구)/후(≈2m/s 유한 속도만 요구) 비교 검증. **`offboard_node.py` 자체는 rclpy 의존이라 이 WSL엔 실행 검증 불가 — 다음 실비행 전 SITL(`gz_x500`) 회귀검증 필수.** 상세는 `docs/session_log.md` 2026-07-20(flight01 제어상실) 항목.
