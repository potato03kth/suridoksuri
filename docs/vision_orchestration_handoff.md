...
status: ⚠️ 2026-07-29 CSI 카메라 물리 사망 → UVC 웹캠 전환 중 (§0-A). §4 백그라운드 트랙은 전부 종료됨
created: 2026-07-28
...
| C. **USB 웹캠(UVC) 임시 경로** | `vision-uvc-webcam` | 🔴 **세션 살아 있음**(잠긴 워크트리) | `b201a27` (07-30 04:38) |

### 0-A-1. 🔴 CSI 카메라가 죽었다 — **USB 웹캠(UVC)으로 대체 중**

**2026-07-29 CSI 카메라(IMX708 클론 CAM109)가 I2C 전 대역 무응답으로 물리적으로 사망**했다
(리본 재체결·재부팅으로 복구 실패, Pi 쪽 결백은 실측으로 확정). 2차예선 마감이 임박해
**다이소 USB 웹캠(UVC)으로 임시 대체**한다. 절차 정본은 **`docs/vision_webcam_fallback.md`**.

**🔴 실물 미검증** — 사용자가 구매하러 간 사이 작성돼 **실물 UVC 장치에서 실행된 적이 없다.**

### 0-A-2. 이 문서에서 **무효가 된 것 / 그대로 유효한 것**

**무효가 된 것 (UVC 는 libcamera 스택을 통째로 우회한다):**
