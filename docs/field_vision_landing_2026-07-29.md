...
> ## 현장 판정 기준
> ### **비행 방향 120 m × 폭 40 m 가 없으면, 편도를 줄이는 것으로는 해결되지 않는다.**
> 편도 25 m 든 60 m 든 **기체는 이륙지점에서 100~115 m 까지 나간다.**
> 그만한 공간이 없으면 줄일 것은 경로가 아니라 **"FW 로 날지 여부"** 다.

- **횡방향도 짧을수록 벌어진다** — 40 m 런에서 **−17.2 m**(60 m 런은 −1.2 m).
- **`range_limit_m` 300 은 어느 길이에서도 안전하다**(실측 최대 112.6 m).
...
- **F2-d(유도 상실 — shim 자신이 죽는 경우) 미실시.** 그 경로는 아직 한 번도 실행된 적이 없다.
...
| 2 | 🔴 **비행 방향 120 m × 폭 40 m 를 확보 못 했다** | 편도를 줄여도 안 준다(§5-1). **FW 로 안 나는 쪽을 택하라** |
