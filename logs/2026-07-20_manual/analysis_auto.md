# 자동분석 — /home/suri/suridoksuri/logs/2026-07-20_manual

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 이 폴더의 다른 ulog (참고, 미분석)
- `log_25_2026-07-20-10-37-04.ulg`: 11.6s
- `log_27_2026-07-20-10-39-34.ulg`: 10.5s
- `log_28_2026-07-20-10-42-10.ulg`: 12.0s
- `log_29_2026-07-20-10-42-48.ulg`: 35.4s

## 주 로그: `log_26_2026-07-20-10-37-50.ulg` (41.06s)
...
### 고도
- home_position AMSL=21.37m, EKF ref_alt AMSL=30.33m
- AGL 최대 0.68m (t=15.66s), 최소 -2.94m (t=35.56s), 종료시 -2.22m

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.5 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 24.31 | 4:AUTO_LOITER | 2:POSCTL | 1 | ARMED | FAILSAFE_FORCED |
| 28.59 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
...
### 자세
- roll 최대|값|=-20.03° (t=32.56s)
- pitch 최대|값|=24.54° (t=30.17s)
- roll 목표(setpoint) 최대|값|=-18.24° (t=10.46s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=6.42s (15.8deg/s)
- pitch rate 이상조짐 후보: t=8.56s (11.1deg/s)
- yaw rate 이상조짐 후보: t=13.5s (9.9deg/s)
> 휴리스틱 후보일 뿐 — 잡음(±5deg/s 이내)에서 15deg/s 이상으로 튄 첫 시점. 이 근처를 §5 얼로케이터 포화 시점·§9 이함 시점과 대조해 진짜 트리거인지 육안 확인할 것.

### 얼로케이터 포화 (축별)
- roll(x): t=1.16s부터 포화 시작 (unallocated=0.0413)
- pitch(y): t=1.16s부터 포화 시작 (unallocated=0.0141)
- yaw(z): t=15.96s부터 포화 시작 (unallocated=-0.0158)
- 통합 achieved 플래그 True 비율: 0.587
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.393 max=0.813 min=0.0
- 1:후좌: mean=0.399 max=0.711 min=0.0
- 2:전좌: mean=0.545 max=1.0 min=0.0
- 3:후우: mean=0.644 max=1.0 min=0.0

### 배터리
- 휴지전압 11.25V, 부하중 최저 8.54V, 전류 최대 37.7A
- remaining: 0.57 → 0.1

### 실패감지기
- imbalanced_prop_metric_max_abs: -17.119
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=3.32s
