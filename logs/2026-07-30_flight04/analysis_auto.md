# 자동분석 — logs/2026-07-30_flight04

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_276_2026-07-30-02-57-44.ulg` (33.43s)
...
### 고도
- home_position AMSL=19.13m, EKF ref_alt AMSL=23.69m
- AGL 최대 8.85m (t=14.16s), 최소 0.01m (t=0.06s), 종료시 1.06m
- notes.md 목표 transition_alt=20.0m → **미달**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.29 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.3 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 17.68 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=-10.19° (t=17.42s)
- pitch 최대|값|=-10.56° (t=5.76s)
- roll 목표(setpoint) 최대|값|=-13.62° (t=16.46s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.11s (-13.7deg/s)
- pitch rate 이상조짐 후보: t=3.99s (8.6deg/s)
- yaw rate 이상조짐 후보: t=4.05s (8.4deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.704 max=1.0 min=0.002
- 1:후좌: mean=0.45 max=0.955 min=0.0
- 2:전좌: mean=0.585 max=1.0 min=0.002
- 3:후우: mean=0.567 max=0.962 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.05V, 부하중 최저 11.61V, 전류 최대 35.0A
- remaining: 0.74 → 0.47

### 실패감지기
- imbalanced_prop_metric_max_abs: -5.02
