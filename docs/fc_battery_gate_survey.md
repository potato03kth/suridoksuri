...
> # 🔴 사용자 결정 (2026-07-29) — **배터리는 무시하고 운용한다. 아래 권고안은 채택하지 않는다.**
>
> 사용자 판단: **"배터리 부분 문제는 해결이 불가하다. 따라서 배터리는 계속 무시하면 되겠다."**
> 팩 정격 용량·내부저항을 확정할 수단이 없어 SoC 추정을 정상화할 수 없다는 것이 근거다.
> 따라서 **§7 권고안(파라미터 3개 변경 + 전하 예산 게이트)은 보류가 아니라 미채택**이다.
> 이 문서는 **"왜 무시해도 되는가"의 근거 자료**로만 남는다.
>
> ## ⚠️ 그러나 — **`COM_LOW_BAT_ACT` 를 절대 켜지 마라**
>
> 지금 설정은 **우연히 안전한 조합**이다. 둘 중 하나만 바꾸면 기체가 못 뜬다:
>
> | 현재 값 | 효과 |
> |---|---|
> | `remaining` 이 이륙 4.5~7.7초에 **항상 0.000** (11건 중 기동 7건 전부, §B-6) | 오탐 |
> | `COM_LOW_BAT_ACT = 0` (Warning only) | 오탐을 **무시** |
> | **합** | 비행 가능 ✅ |
>
> 누군가 "배터리 페일세이프가 꺼져 있네"라며 `COM_LOW_BAT_ACT` 를 `2`(RTL)나 `3`(Land)으로
> 켜면 — `BAT1_CAPACITY`·`BAT1_R_INTERNAL` 을 **함께** 고치지 않는 한 —
> **만충 팩으로도 모든 비행이 이륙 7초에 자동 착륙한다.** 대회 당일이면 기체가 못 뜬다.
>
> ⇒ 배터리 잔량은 **PX4 지표를 믿지 말고 비행 시간·무부하 전압으로 운용자가 직접 관리**한다.
> `/mavros/battery` 의 `0%` 도 같은 이유로 무시한다(비행 내내 0 으로 고정된다).
>
> 후속 세션은 이 결정을 뒤집는 제안(게이트 도입·파라미터 교정)을 **다시 꺼내지 말 것.**
> 사용자가 먼저 배터리를 다시 논의 대상으로 올리기 전까지는 종결된 사안이다.
