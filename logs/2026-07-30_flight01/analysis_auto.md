# 자동분석 — logs/2026-07-30_flight01

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_268_2026-07-30-01-13-14.ulg` (13.76s)
...
### 고도
- home_position AMSL=17.49m, EKF ref_alt AMSL=19.72m
- AGL 최대 0.39m (t=13.66s), 최소 0.0m (t=0.06s), 종료시 0.39m
...
| 4.85 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 12.76 | 2:POSCTL | 2:POSCTL | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=2.98° (t=7.87s)
- pitch 최대|값|=-2.74° (t=7.92s)
- roll 목표(setpoint) 최대|값|=-10.22° (t=12.76s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=7.81s (22.2deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.426 max=1.0 min=0.002
- 1:후좌: mean=0.109 max=0.579 min=0.0
- 2:전좌: mean=0.246 max=0.819 min=0.002
- 3:후우: mean=0.312 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.14V, 부하중 최저 15.22V, 전류 최대 13.0A
- remaining: 0.81 → 0.81

### 실패감지기
- imbalanced_prop_metric_max_abs: -0.588
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=5.82s
