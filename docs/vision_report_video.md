...
> 🟡 **2026-07-31 — 예비 경로가 하나 준비돼 있다(발동 안 됨).** 리본 재체결·재부팅으로 복구되지
> 않아 모듈 자체 문제로 판정됐고, **모듈 교체로 이 문서의 경로 A를 복구하는 것이 1순위**다.
> 그게 막혔을 때를 대비해 **USB 웹캠(UVC) 경로**를 미리 구현해 뒀다 —
> **`docs/vision_webcam_fallback.md`**(발동 조건은 그 문서 §0). 발동하면 아래 §2 "경로 A"의
> `live`를 `uvc`로 바꾸는 것으로 이 문서의 나머지 절차가 그대로 적용된다.
> 중간에 검토한 휴대폰 IP Webcam 안은 사용자 실측 결과 **화질 부족으로 기각**됐다.
