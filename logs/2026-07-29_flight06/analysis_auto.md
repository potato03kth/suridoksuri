# 자동분석 — logs/2026-07-29_flight06

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 이 폴더의 다른 ulog (참고, 미분석)
- `log_246_2026-07-29-11-22-44.ulg`: 10.5s

## 주 로그: `log_248_2026-07-29-11-24-04.ulg` (68.08s)
...
### 고도
- home_position AMSL=24.38m, EKF ref_alt AMSL=20.48m
- AGL 최대 19.99m (t=24.27s), 최소 -1.24m (t=68.07s), 종료시 -1.24m
- notes.md 목표 transition_alt=20.0m → **달성**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.5 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 1.03 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 19.71 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 22.73 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 29.52 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=22.4° (t=31.72s)
- pitch 최대|값|=-29.67° (t=38.47s)
- roll 목표(setpoint) 최대|값|=7.09° (t=33.12s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.05s (-8.3deg/s)
- pitch rate 이상조짐 후보: t=4.25s (8.3deg/s)
- yaw rate 이상조짐 후보: t=4.37s (8.2deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.702 max=1.0 min=0.002
- 1:후좌: mean=0.745 max=1.0 min=0.0
- 2:전좌: mean=0.643 max=0.965 min=0.002
- 3:후우: mean=0.695 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.43V, 부하중 최저 11.04V, 전류 최대 32.4A
- remaining: 0.43 → 0.17

### 실패감지기
- imbalanced_prop_metric_max_abs: -6.386
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=1.98s
