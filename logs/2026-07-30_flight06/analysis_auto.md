# 자동분석 — logs/2026-07-30_flight06

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_280_2026-07-30-03-00-34.ulg` (22.58s)
...
### 고도
- home_position AMSL=20.7m, EKF ref_alt AMSL=23.8m
- AGL 최대 5.92m (t=12.27s), 최소 0.0m (t=0.06s), 종료시 0.04m
- notes.md 목표 transition_alt=20.0m → **미달**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.37 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.38 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 13.87 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=-13.15° (t=13.22s)
- pitch 최대|값|=-16.37° (t=12.62s)
- roll 목표(setpoint) 최대|값|=-14.84° (t=12.96s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.13s (-12.6deg/s)
- pitch rate 이상조짐 후보: t=4.75s (10.5deg/s)
- yaw rate 이상조짐 후보: t=4.17s (9.0deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.8 max=1.0 min=0.002
- 1:후좌: mean=0.531 max=1.0 min=0.0
- 2:전좌: mean=0.651 max=1.0 min=0.002
- 3:후우: mean=0.601 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.19V, 부하중 최저 10.94V, 전류 최대 37.1A
- remaining: 0.33 → 0.19

### 실패감지기
- imbalanced_prop_metric_max_abs: -4.893
