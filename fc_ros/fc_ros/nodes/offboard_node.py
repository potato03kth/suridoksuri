...
class OffboardNode(Node):
    ...
    def _path_end_dir(self) -> np.ndarray:
        ...
        ⚠️ **`self._end_dir` 를 재사용하지 않는다.** 그쪽은 원본 waypoints 의
        마지막 레그(현(弦))이고 역천이 직진용이다. 결승선은 `_pts[-1]` 을
        기준점으로 쓰므로 법선도 **같은 배열의 접선**에서 나와야 한다 —
        곡선 플래너(eta3)에서 현과 접선은 어긋날 수 있고, 기준점과 법선을
        서로 다른 출처에서 가져오는 것이 이 저장소가 반복해서 당한 실패
        유형이다. 게다가 `_end_dir` 은 퇴화 시 조용히 `[1,0]`(정북) 으로
        떨어지는데, 그건 여기서 **임의 방향의 오완료**가 된다.
        """
        pts = self._pts
        n = len(pts)
        if n < 2:
