# 자동분석 — logs/2026-07-30_flight05

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_279_2026-07-30-02-59-26.ulg` (72.93s)
...
### 고도
- home_position AMSL=20.28m, EKF ref_alt AMSL=23.69m
- AGL 최대 16.27m (t=42.17s), 최소 -1.68m (t=71.17s), 종료시 -1.63m
...
| 0.53 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 53.1 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=16.98° (t=17.02s)
- pitch 최대|값|=-14.09° (t=5.97s)
- roll 목표(setpoint) 최대|값|=17.33° (t=16.37s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=3.85s (-8.1deg/s)
- pitch rate 이상조짐 후보: t=4.57s (9.6deg/s)
- yaw rate 이상조짐 후보: t=4.13s (8.5deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.793 max=1.0 min=0.002
- 1:후좌: mean=0.793 max=1.0 min=0.0
- 2:전좌: mean=0.804 max=1.0 min=0.002
- 3:후우: mean=0.838 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.81V, 부하중 최저 11.0V, 전류 최대 37.7A
- remaining: 0.59 → 0.2

### 실패감지기
- imbalanced_prop_metric_max_abs: -6.281
