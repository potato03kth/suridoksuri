...
올바른 방향 결선도 확인했다."* 이를 전류 실측(**+0.81 A**)·전진속도(**u ≈ 0**)와 함께 놓으면
남는 설명은 하나다 — **모터는 돌았으나 프로펠러가 공기를 물지 못했다.**

- 4S 프로펠러 모터가 부하를 제대로 받으면 전류가 수십 A 급증한다. +0.81 A 는 **무부하 회전**의 값이다.
- ⚠ **모터 회전방향이 맞는 것과 프로펠러가 올바로 장착된 것은 별개다.** 프롭 앞뒤 뒤집힘,
  피치 부족·손상, 폴딩 프롭 미전개는 전부 "돌지만 추력 없음 + 저전류"를 만든다.
