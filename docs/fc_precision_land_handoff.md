...
status: ⏸ SITL 완료(c·d·f·g 전부 실증) · 실비행 0건 — 천이가 성립하지 않아 HOLD 에 도달 못 함. 차단 해제는 ✈ vtol-실기체 지상 조치 (§0-A)
created: 2026-07-28
...
## 0-A. 🔴 2026-07-31 갱신 — **F2 실비행은 0 건이고, 차단 원인은 F2 밖에 있다**

이 문서는 **2026-07-29 11:28(`3d73bfb`)에서 멈춰** 있었다. 그 뒤 SITL 검증 3 세션과 실비행 2 일치가
진행됐다. **§0 보다 이 절이 먼저다.**

### 0-A-1. ✅ SITL 은 닫혔다 — 결함 c·d·f·g 전부 실증

`b9b8f8f`·`a6d2072`·`94ef01b` (2026-07-29, 격리 클론을 그날 나는 코드 `44027a5` 로 올려 재빌드):

| | 결과 |
|---|---|
| **F2-f** | ✅ align 30 s 타임아웃 → 나선 강제 진행 (V6) |
| **F2-c** | ✅ 생산자 사망을 t=13/60 s 에 잡아 GPS 폴백 (V5b) |
| **F2-g** | ✅ 인계고도 3.5 m±0.5 정상 인계 — V2 의 "3.1 m 에서 22 초 정지"가 사라졌다 |
| **F2-d** | ✅ **이 경로 첫 실행.** `vision_lost_timeout=5.0` 실제 발화, 실측 5.83 s(stale 1.0 + 상실 5.0). GPS 폴백 → 21.4 s 뒤 DONE. **폴백 직전 2.7 초는 고도를 붙들었다**(17.7→16.4 m) = `descend_allowed(guided=False)` 의 "추측 하강 금지" 계약대로 |
| **`vision_landing=false` 후퇴 경로** | ✅ HOLD → LANDING → DONE 완주(exit=0 done 119.5 s). `VISION_SEARCH`/`PRECISION_LAND` 미등장. **F2-e 계약 재확인** — 꺼져 있으면 거리상한 문구·판정 무변경 |

**🔴 F2-c 와 F2-d 는 현장에서 로그로 구분된다** — F2-c = "생산자 사망(link ERROR)" /
F2-d = "유도 상실 5 s 지속"(link ERROR 가 한 줄도 없고 status age=6.0 s 가 증거).
`vision_target_bridge.py:74-77` 페일세이프 3분법이 설계대로 갈렸다.

### 0-A-2. 🔴 그런데 실비행은 **0 건**이다 — 그리고 F2 탓이 아니다

**07-29 11 건 · 07-30 7 건의 어느 `launch.log` 에도 `비전 정밀착륙 활성` · `VISION_SEARCH` ·
`PRECISION_LAND` 가 한 줄도 없다** = `vision_landing` 이 켜진 적이 없다(전수 확인).

F2 는 `HOLD` **이후**에 발동한다. 거기까지 못 갔다:

- **07-30 f02** — 천이 시도 → **Quad-chute**(t=38.30). 원인은 **피토 영점 21 Pa 오차**
  (`SENS_DPRES_OFF=−21.78` vs 실제 −42.6). 실제 11.2 m/s 를 PX4 가 9.56 으로 저평가 →
  `VT_ARSP_TRANS=12.0` 미달. `vehicle_vtol_state` 는 3→1→3 으로 **FW(4) 에 한 번도 못 닿았다.**
- **나머지 6 건** — 목표 `transition_alt=20 m` 미달(5.9~15.7 m). **T/W ≈ 1.44** 라 상승 여유가 없다.
  **f07 은 14.1 m 에서 무지령 강하 → 8.2 G 접지(추락).**

**→ 차단 해제는 이 트랙이 아니라 ✈ vtol-실기체 트랙의 지상 조치다:**
① 피토 영점 재캘리(`SENS_DPRES_OFF` → −42.6 근방) ② `VT_ARSP_TRANS` 12.0 → 10.0
③ CG 조정(배터리·페이로드를 뒤·왼쪽으로 — 앞/뒤 추력비 1.55 배).
상세는 `docs/fc_orchestration_handoff_2026-07-29.md` §0-A-1 · `logs/2026-07-30_review.md`.

> **🔴 "SITL 에서 c·d·f·g 가 전부 닫혔다"를 "실기체에서 검증됐다"로 읽지 말 것.**
> F2 코드가 실기체에서 한 틱도 실행된 적이 없다.

### 0-A-3. 운용 파라미터가 바뀌었다 — 이 문서의 옛 값 주의

`fc8631d`(2026-07-29 사용자 지시)로 **`transition_alt` 50 → 25 m** / **`waypoints` 300 → 150 m 편도**
(z 도 25) / **`vision_search_timeout` 120 → 180 s**. 아래 §8 파라미터 레퍼런스와 §0 이하 본문에
**50 m·300 m·120 s 를 전제한 서술이 남아 있으면 그것은 낡은 값이다.**

`transition_alt=25` 의 효과는 F2 에 직접적이다 — **VISION_SEARCH 진입 0.2~0.6 s 만에 래치**하고,
50 m 래치 시 하강만 58.2 s 라 `precision_land_timeout`(60 s)를 거의 다 쓰던 문제가 27.5 s 로 준다.

### 0-A-4. 🔴 F2 실비행 전 재확인 대상 — **카메라가 바뀌었다**

**2026-07-29 CSI 카메라가 물리적으로 사망**해 **USB 웹캠(UVC)으로 임시 대체** 중이다(실물 미검증).
F2 는 vision 출력에 의존하므로 **화각·`camera_matrix`·발행 주파수가 전부 재측정 대상**이다.
특히 §7 의 잠정값과 `vision_stale_timeout` 계약은 **CSI 4608×2592 기준 4.4 Hz 실측** 위에 서 있다.
상세는 `docs/vision_webcam_fallback.md` · `docs/vision_orchestration_handoff.md` §0-A.

### 0-A-5. 재현 스크립트 형상 주의

`f2c_v8.sh`/`f2c_v7.sh`/`f2c_v7b.sh` 는 **앞 세션 형상(300 m / alt50 / 상한 1200)** 이라 그대로 쓰면
안 된다. 현재 형상판은 **`f2c_v8b.sh`/`f2c_v7b.sh`**. 환경 함정: **gz sim 이 안 뜨면 PX4 가 6 분 뒤
logger 스레드 segfault 로 죽는다**(코드와 무관).
...
### 8-2. 🔴 2026-07-29 후속 결함 7건 — F2 가 "동작한다"와 "믿을 수 있다" 사이

`9c5d17f`(블로커 2건 수정)로 F2 는 **동작하게** 됐고, 그 검증(`6e3ea0c`) 과정에서
아래 7건이 드러났다(g 는 a~f 를 고친 뒤 SITL 에서 새로 드러났다). 전부 "죽지는 않지만 틀린 판단을 하거나 시간을 태우는" 결함이다.
...
| **g** | 🔴 **해피패스의 정상 종료가 타임아웃이었다** (a~f 를 고친 뒤 SITL 에서 **새로 드러난** 7번째). `_step_precision_land` 의 하강 스케줄 하한이 `vision_land_handoff_agl` 과 **정확히 같은 숫자**(3.0m)라, FC 는 목표를 3.0m 에 놓고 기체는 위치제어 정상오차만큼 그 **위에** 정착한다 → `handoff_due` 의 `agl <= 3.0` 단측 판정이 **원리적으로 성립하지 않는다.** 실측 `V2_vision_happy`: AGL 3.4m 까지 정상 하강 후 **3.1m 에서 22초 정지** → 인계 대신 `precision_land_timeout` → GPS 폴백. 검증 세션 `R5` 가 못 본 이유는 `command_hint="land"` 가 42s 에 먼저 구해줬기 때문이다(②번 경로) | 판정을 점이 아닌 **허용대**(±0.5m)로. 새로운 종류의 실수가 아니라 **이 저장소가 이미 한 번 고친 것**이다 — `climbing_reached()` 가 "AGL >= transition_alt" 단측 판정으로 CLIMBING 무한대기를 겪고 `alt_tol` 구간으로 바꿨다. 허용대 0.5m 도 그쪽 기본값과 같은 숫자다. 인계가 3.0 → 3.5m 로 **높아지는** 방향이라 안전측이다 |
...
> **회귀 그물:** `fc_bridge/tests/test_precision_land.py`(순수 판정 17건 추가) +
> `fc_ros/test/test_offboard_f2_state.py`(노드 전이 43건 추가). 파괴검증 11종
