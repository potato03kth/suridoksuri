...
docker exec fc bash -lc '
  ...
  ros2 launch fc_ros phase2.launch.py vision_landing:=true \
    waypoints:="[0.0, 0.0, 25.0, 60.0, 0.0, 25.0]"
'
```

### ✅ 편도 **60 m 확정** (2026-07-29 사용자 결정)

현장이 비행 방향 **150 m** 로 확인됐다. 편도 60 m 의 관측 최대 이탈이 **122.6 m** 이므로
**여유 27.4 m**. 그리고 **편도 60 m 는 이 세션에서 표본 3건으로 가장 잘 덮인 길이다**
(`S60_short` 완주 · `V7b` F2-d 실증 · `V8b` 후퇴경로 완주). `transition_alt` 는 yaml 기본 25.0
이 맞으므로 **인자로 줄 필요 없다.**

**이륙 직전 로그 한 줄을 반드시 눈으로 확인한다** (F2-e 가 이걸 위해 있다):

```
거리 상한 감시 활성 300m (이륙지점 기준 수평, 경로 최원점 60m, 탐색 최원점 90m (=WP1 60m + 반경 30m))
```

**`탐색 최원점 90m` 이 안 찍히면 `vision_landing` 이 안 켜진 것이다** — 그 줄이 F2 활성화의
가장 빠른 확인 수단이다(§6-1 게이트보다 먼저 보인다).
