# 자동분석 — logs/2026-07-21_flight09

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_41_2026-07-21-11-29-46.ulg` (37.55s)
...
### 고도
- home_position AMSL=25.5m, EKF ref_alt AMSL=24.24m
- AGL 최대 1.35m (t=14.86s), 최소 -1.15m (t=37.46s), 종료시 -1.15m
...
| 1.83 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 6.42 | 4:AUTO_LOITER | 17:AUTO_TAKEOFF | 1 | ARMED | FAILSAFE_FORCED |
| 11.33 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 33.18 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=-18.37° (t=26.76s)
- pitch 최대|값|=18.18° (t=31.26s)
- roll 목표(setpoint) 최대|값|=-10.25° (t=17.71s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.48s (19.7deg/s)
- pitch rate 이상조짐 후보: t=4.66s (10.9deg/s)
- yaw rate 이상조짐 후보: t=4.76s (12.3deg/s)
> 휴리스틱 후보일 뿐 — 잡음(±5deg/s 이내)에서 15deg/s 이상으로 튄 첫 시점. 이 근처를 §5 얼로케이터 포화 시점·§9 이함 시점과 대조해 진짜 트리거인지 육안 확인할 것.

### 얼로케이터 포화 (축별)
- roll(x): t=1.16s부터 포화 시작 (unallocated=0.0729)
- pitch(y): t=1.16s부터 포화 시작 (unallocated=0.0122)
- yaw(z): t=1.16s부터 포화 시작 (unallocated=0.0018)
- 통합 achieved 플래그 True 비율: 0.441
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.455 max=1.0 min=0.002
- 1:후좌: mean=0.489 max=1.0 min=0.0
- 2:전좌: mean=0.64 max=1.0 min=0.002
- 3:후우: mean=0.737 max=1.0 min=0.0

### 배터리
- 휴지전압 12.36V, 부하중 최저 8.61V, 전류 최대 39.3A
- remaining: 1.0 → 0.25

### 실패감지기
- imbalanced_prop_metric_max_abs: -16.36
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=2.77s
