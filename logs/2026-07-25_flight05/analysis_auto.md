# 자동분석 — logs/2026-07-25_flight05

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_158_2026-07-25-09-59-06.ulg` (12.99s)
...
### 고도
- home_position AMSL=20.74m, EKF ref_alt AMSL=26.59m
- AGL 최대 0.35m (t=12.26s), 최소 -0.01m (t=2.46s), 종료시 0.33m
- notes.md 목표 transition_alt=3.0m → **미달**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.29 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.31 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 11.99 | 2:POSCTL | 2:POSCTL | 0 | STANDBY | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=-8.59° (t=5.31s)
- pitch 최대|값|=15.7° (t=5.41s)
- roll 목표(setpoint) 최대|값|=-6.58° (t=10.25s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.42s (-9.4deg/s)
- pitch rate 이상조짐 후보: t=4.54s (12.4deg/s)
- yaw rate 이상조짐 후보: t=4.54s (-11.3deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.602 max=1.0 min=0.002
- 1:후좌: mean=0.44 max=1.0 min=0.0
- 2:전좌: mean=0.513 max=0.966 min=0.002
- 3:후우: mean=0.409 max=0.921 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.71V, 부하중 최저 13.81V, 전류 최대 26.9A
- remaining: 1.0 → 0.99

### 실패감지기
- imbalanced_prop_metric_max_abs: -1.667
