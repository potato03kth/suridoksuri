...
4. **종료:** 세션 종료 전 `/session-log` — 트랙 블록 갱신 + **handoff 갱신** + 서술 로그 기록 + 아카이브. 급하면 `/session-log 축약`(트랙 보드·handoff 는 축약해도 한다). 정의는 `.claude/commands/session-log.md`.
   - **오케스트레이션 세션(서브에이전트를 띄웠거나 "너는 오케스트레이터이다"로 시작)은 handoff 갱신이 트랙 보드보다 중요하다** — 결정 대기·목적·미실시 사유·함정은 **커밋 메시지에 원리적으로 안 담긴다**.
   - **절차를 실행했다고 세션이 닫히는 게 아니다.** 워크트리가 잠긴 채 프로세스가 살아 있으면 다음 세션이 "진행 중"으로 읽는다 — 실제로 종료할 것.
   - 🔴 이 단계가 빠지면 **아무도 모른다.** `session_log.md` 는 진입 경로가 아니라 종료 산출물이라 며칠 비어도 티가 안 난다 (2026-07-28~30 에 74커밋이 그렇게 누락됐고, 그 사이 트랙 보드의 운용 파라미터가 낡아 다음 세션이 오독할 뻔했다).
