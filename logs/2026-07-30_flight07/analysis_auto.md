# 자동분석 — logs/2026-07-30_flight07

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_282_2026-07-30-03-02-34.ulg` (32.07s)
...
### 고도
- home_position AMSL=20.26m, EKF ref_alt AMSL=23.8m
- AGL 최대 14.11m (t=20.17s), 최소 -2.63m (t=26.56s), 종료시 -0.68m
...
| 0.61 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 22.38 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=28.67° (t=24.11s)
- pitch 최대|값|=-11.69° (t=23.76s)
- roll 목표(setpoint) 최대|값|=-17.45° (t=24.46s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=4.18s (-12.3deg/s)
- pitch rate 이상조짐 후보: t=4.49s (11.8deg/s)
- yaw rate 이상조짐 후보: t=4.33s (10.1deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.817 max=1.0 min=0.002
- 1:후좌: mean=0.578 max=1.0 min=0.0
- 2:전좌: mean=0.728 max=1.0 min=0.002
- 3:후우: mean=0.656 max=0.998 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.07V, 부하중 최저 9.36V, 전류 최대 34.5A
- remaining: 0.29 → 0.0

### 실패감지기
- imbalanced_prop_metric_max_abs: -4.54
