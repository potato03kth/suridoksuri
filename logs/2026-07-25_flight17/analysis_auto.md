# 자동분석 — logs/2026-07-25_flight17

(analyze_flight.py 자동생성 — 해석 없이 사실만. notes.md에 결론 작성 시 이 파일 인용)

## 주 로그: `log_201_2026-07-25-11-22-34.ulg` (72.28s)
...
### 고도
- home_position AMSL=19.73m, EKF ref_alt AMSL=22.0m
- AGL 최대 20.19m (t=23.56s), 최소 0.0m (t=0.06s), 종료시 2.24m
- notes.md 목표 transition_alt=20.0m → **달성**

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.32 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
| 0.33 | 17:AUTO_TAKEOFF | 17:AUTO_TAKEOFF | 0 | ARMED | INTENTIONAL_CHANGE |
| 18.26 | 4:AUTO_LOITER | 4:AUTO_LOITER | 0 | ARMED | INTENTIONAL_CHANGE |
| 21.32 | 14:OFFBOARD | 14:OFFBOARD | 0 | ARMED | INTENTIONAL_CHANGE |
| 42.72 | 2:POSCTL | 2:POSCTL | 0 | ARMED | INTENTIONAL_CHANGE |
...
### 자세
- roll 최대|값|=26.32° (t=44.57s)
- pitch 최대|값|=17.57° (t=44.17s)
- roll 목표(setpoint) 최대|값|=-31.64° (t=42.51s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=1.79s (12.9deg/s)
- pitch rate 이상조짐 후보: t=4.21s (13.4deg/s)
- yaw rate 이상조짐 후보: t=4.63s (8.8deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.649 max=1.0 min=0.002
- 1:후좌: mean=0.621 max=1.0 min=0.0
- 2:전좌: mean=0.702 max=0.99 min=0.002
- 3:후우: mean=0.611 max=1.0 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 16.1V, 부하중 최저 12.53V, 전류 최대 39.3A
- remaining: 0.73 → 0.39

### 실패감지기
- imbalanced_prop_metric_max_abs: -8.442
