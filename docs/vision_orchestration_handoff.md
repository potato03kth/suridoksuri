...
track: 👁 vision-정밀착륙
scope: 2026-07-28 vision 오케스트레이션 세션 인수인계 — 이 문서 하나로 다음 세션이 완전히 이어받는다 (2026-07-31: 미종료 세션 2건 + 카메라 교체 소급 반영)
status: ⚠️ 2026-07-29 CSI 카메라 물리 사망 → 🔴 **모듈 교체로 CSI 복구가 1순위**, UVC 웹캠은 🟡 조건부 예비(발동 안 됨, §0-A). §4 백그라운드 트랙은 전부 종료됨
created: 2026-07-28
...
## 0-A. 🔴 2026-07-29~31 추가분 — **이 문서의 전제가 바뀌었다 (카메라 교체)**

이 문서는 **2026-07-28 18:52(`9309b29`)에서 멈춰** 있었다. 그 뒤 vision 오케스트레이션 세션 2건이
진행됐고 그중 하나가 **이 문서 전체의 하드웨어 전제를 무효화**했다. **§0-a 보다 이 절이 먼저다.**

| 세션 | 워크트리 | 종료 상태 | 커밋 |
|---|---|---|---|
| D. 보고 제출용 영상 녹화 경로 정비 | `vision-demo-video` | ✅ **닫힘**(`vision_status.md` 갱신함) | `190629e` (07-29 17:22) |
| C. **USB 웹캠(UVC) 🟡 예비 경로**(발동 안 됨) | `vision-uvc-webcam` | ✅ **닫힘**(07-31 지위 정정 커밋으로 종료) | `b201a27`·`685a2ed` |

### 0-A-1. 🔴 CSI 카메라가 죽었다 — **모듈 교체로 복구하는 것이 1순위**

**2026-07-29 CSI 카메라(IMX708 클론 CAM109)가 I2C 전 대역 무응답으로 물리적으로 사망**했다
(리본 재체결·재부팅으로 복구 실패, Pi 쪽 결백은 실측으로 확정).

🔴 **2026-07-31 사용자 확인 — 기본 경로는 여전히 라파(CSI) 카메라다.** 원문: *"USB 웹캠으로의
대체작업은 **어쩌면 해야할 지도 모르는** 작업이다. 그 외의 경우에는 **기존대로 라파카메라로
진행**할 것."* → **모듈 교체 후 기존 스택 그대로 간다**(libcamera 소스빌드·picam-venv·
`LiveFrameSource` 전부 유효, 다시 만들 필요 없다).

🟡 **USB 웹캠(UVC)은 조건부 예비책이고 발동되지 않았다** — 웹캠을 사지도, 실행하지도 않았다.
코드(`UvcFrameSource` + `main.py uvc:<장치>`)만 미리 병합해 뒀는데, `uvc`가 opt-in 특수값이라
안 쓰면 한 줄도 실행되지 않고 `_run_live` 시그니처·동작이 무변경이라 **기본 경로 비용이 0**이기
때문이다. 발동 조건·발동 시 절차·잃는 것 목록은 **`docs/vision_webcam_fallback.md` §0**.

⚠️ **이 절의 예전 판("UVC 로 대체 중")은 07-30 세션의 오기였고 07-31에 정정됐다.** 아래 0-A-2
"무효가 된 것"도 **발동했을 때만** 무효라는 뜻으로 읽을 것 — 지금은 §3 내용이 전부 유효하다.

**🔴 실물 미검증** — 웹캠 자체가 없어 **실물 UVC 장치에서 실행된 적이 없다.** 발동해도
실물 확인 + 화각 실측 + `nominal.yaml` 산출 리드타임이 앞에 붙는다(= 즉시 사용 가능 아님).

### 0-A-2. 🟡 **웹캠이 발동됐을 때만** 무효가 되는 것 / 그대로 유효한 것

> ⚠️ 발동 전(=현재)에는 아래 "무효" 항목이 **전부 그대로 유효하다.** CSI 로 계속 가기 때문이다.

**발동 시 무효가 되는 것 (UVC 는 libcamera 스택을 통째로 우회한다):**

- **§3-1 런타임 환경 중 "picam-venv(호스트 Py3.12) 분리가 필요하다"** — UVC 는 커널이 `/dev/video*` 로
  바로 잡고 `cv2.VideoCapture` 로 열린다. `ipa_rpi_pisp.so` 소스빌드도, `env.sh` source 도,
  picam-venv 분리도, `/dev/mediaN` 번호 변동도, 카메라 배타성도 **전부 해당 없다.**
  부수 효과로 **랩탑 `.venv` 에서 그대로 돌아** 실기체 없이 개발·검증된다.
- **§3-5 "발행 주파수는 4.4 Hz"** — 그 수치는 **CSI 4608×2592 실측**이다. UVC 는 센서·해상도·
  전송경로가 전부 달라 **재측정 대상**이다. 이 값에 의존하는 계약(`vision_stale_timeout` 등)을
  UVC 실측 전에 그대로 믿지 말 것.

**그대로 유효한 것 (재조사 금지 유지):**

- **§3-1 의 핵심 결론 — 호스트에 ROS 2 가 없다 → 소켓 + 컨테이너 shim.** 카메라가 무엇이든 이 구조는
  안 바뀐다. `--network host` 라 `127.0.0.1` 이 그대로 통하는 것도 동일.
- **§3-2 `mavros_msgs/LandingTarget` frame 상수 off-by-one**, **§3-3 인터페이스 계약**,
  **§3-4 FC 도메인 경계**, **§3-6 CLOCK_MONOTONIC 동일** — 전부 카메라와 무관.

### 0-A-3. 🔴 UVC 로 넘어갈 때 반드시 지킬 안전장치 — **해상도 되읽기**

싸구려 웹캠은 **지원하지 않는 해상도를 요청받으면 에러 대신 조용히 다른 해상도를 돌려주고
`cap.set()` 은 True 를 반환한다.** `camera_matrix` 가 해상도에 묶여 있어(2026-07-29 에 실제로
1920 프레임 + 4608 캘리브로 거리가 **2.4 배** 틀렸던 그 사고) 그대로 두면 **거리·pose 가 소리 없이
배수로 틀린다.** → 여는 즉시 되읽어 **하드 실패**시키고 에러에 조치 명령 3종을 담는다.
**재시도 대상이 아니다.**

> **오해 방지.** `vision/utils/frame_source.py` docstring 의 *"`cv2.VideoCapture` 는 V4L2 raw 경로와
> 비호환(`isOpened()` 는 되는데 `read()` 가 실패)"* 는 **CSI 베이어 raw** 에 대한 기록이고 UVC 와 무관하다.
> UVC 는 드라이버가 디베이어·포맷변환을 끝낸 프레임을 준다. **그 전례를 근거로 이 경로를 되돌리지 말 것.**

### 0-A-4. 해상도 요구 — 초록구역은 VGA 로도 된다

고도별 타겟 픽셀 실산출: **초록매트 3.0 m 는 VGA 로도 40 m 에서 42 px** 로 충분하다(우선 타겟이라
싸구려 웹캠으로 커버된다). **ArUco 0.5 m 는 FHD 로도 40 m 에서 21 px** 로 디코드 한계 밑 →
**저고도(10 m, 83 px) 전용**이다. 참고로 CAM109 도 40 m 에서 38 px 로 여유롭진 않았다.

### 0-A-5. 기각된 대안

**휴대폰 IP Webcam(MJPEG over HTTP)** — 사용자가 실제로 시험했으나 **화질 부족으로 기각**.
무선이라 현장 2.4 GHz 링크(RC)와 대역을 다투는 문제도 있었다. USB 웹캠은 그 두 문제가 다 없다.

### 0-A-6. ⚠️ 아래 §0-a·§4 의 "돌고 있다"는 **이미 끝난 이야기다**

프런트매터 `status` 와 §0-a 가 *"2차 웨이브 백그라운드 트랙 3개(A/B/C)가 돌고 있다"*고 적고 있으나,
같은 문서 **§4-0 이 "4트랙 전부 종료·오케스트레이터 재현 검증 완료"**라고 적는다 — 본문이 헤더를
이미 추월했다. **§4 는 완료 기록으로 읽어라.** 2026-07-31 에 프런트매터를 정정했다.
...
### ✅ 4-0. 2차 웨이브 — 2026-07-28 설계 논의 후속 (4트랙 **전부 종료·오케스트레이터 재현 검증 완료**)

§5 6~10번 결정을 구현한 트랙이다. **`pytest vision/tests/` 886 → 1028 passed, 회귀 0.**

| # | 트랙 | 커밋 | 오케스트레이터 재현 검증 |
|---|---|---|---|
| **A** | **착륙점 기하 재설계** — `interior_margin_ratio` 폐기(인자는 받되 무시+`DeprecationWarning`), 기체반경 `R` 기반 안전창에서 `d`를 도출. 창이 비면 지어내지 않고 거절 | `bfd6cca` | ✅ 안전창 부등식 손계산 대조(R=0.3/0.5/0.6/0.6444/0.7/1.0 전건 일치) · **절벽 `R_max` 독립 유도 0.644365 = 코드 수출값**(소수 6자리) · 기본값 파괴 D7(R 0.60→0.40)→4 red / D8(`MEASURED`→True)→2 red / 원복 green |
| **B** | **shim setpoint 변환** — `/mavros/local_position/pose` 구독 → `/vision/landing_setpoint`(ENU `PoseStamped`) | `27dffa3` `8ec3c84` | ✅ 방향쌍 5건 손계산 일치(북/동/남/서 + 오프셋) · **드리프트 상쇄**(기체 (100,200,30)→(105,195,31) 이동 시 목표 이동량 = 기체 이동량) · `build_landing_setpoint`가 `pose`를 인자로 받는 **순수 함수라 절대좌표 캐시가 구조적으로 불가능** · 부호 파괴 DS-1(켤레)→6 red, DS-5(N/E)→3 red |
| **C** | **사전정보 스코어링** — 호모그래피 예상 형상으로 후보 랭킹. 거절 금지, 입력 없으면 자동 비활성 | `bbd1413` | ✅ `cos³θ` 법칙 손계산 일치(10°→0.9551, 25°→0.7444) · `fx=1000.8771` 일치 · **종단간: 자세 있음 z=20.8339m(진짜) / 자세 없음 z=13.2347m(4.5m 오탐을 3.0m로 오인)** · 거절금지 파괴→5 red |
| **D** | **TERMINAL 밀어붙이기** — `ABORT_ASCEND` 2경로를 3분기로 대체, 임계를 `max_drift_estimate_m`과 분리 | `256dd42` | ✅ **레거시 등가성을 변경 이전 커밋(`8ec3c84`) 코드를 실제 실행해 대조 — 두 경로 모두 완전일치** · 신 기본 동작에 `ABORT_ASCEND` 0건 · D3(`allow_terminal_abort_ascend` 기본값→True)→11 red |

**D의 TERMINAL 3분기 (재현 확인):**

| 조건 | state | command | reason |
|---|---|---|---|
| 이탈 ≤ 임계, 블라인드 ≤ 2.0s | `TERMINAL` | `descend` | `terminal_blind_deadreckoning` (무변경) |
| 이탈 ≤ 임계, 블라인드 > 2.0s | `TERMINAL` | **`land`** | `terminal_commit_handoff_blind_timeout` |
| 이탈 > 임계 (**시간 무관 — 기하 게이트가 타이머를 이긴다**) | **`HOLD`** | `hold` | `terminal_hold_drift_exceeds_commit` |

`descend`가 아니라 `land`인 이유: 2초째 못 보는 추정으로 횡방향을 계속 물고 늘어지는 것이 오차를
**키우는** 모드다(접지 순간 횡속도 → 라이즈드 가장자리 전복). `closed_loop_floor_agl_m`=3.0m가
`terminal_agl_m`과 **같은 숫자**라, 기존 "3m까지 정렬 후 AUTO.LAND 인계" 계약을 발화하는 셈이다.

### 🔴 4-0a. 이 웨이브가 드러낸 것 — 3m 매트에는 **기하 절벽**이 있다

`d ≥ b + R/√2`(박스 회피, 대각선으로 잰다) 와 `d ≤ M − R − δ`(매트 이탈, **축 방향**으로 잰다)를
R에 대해 풀면:

```
R_max = (M − δ − b)/(1 + 1/√2)        M=1.5, b=0.10(20cm 박스 반변)
δ=0.30 → 0.644    δ=0.20 → 0.703    δ=0.10 → 0.762    δ=0.00 → 0.820
```

- **현재 R 공칭값 0.60은 절벽에서 4.4cm 앞이다.** 실측이 이걸 넘으면 3m 매트 위에 "박스 옆"
  착륙점이 **아예 존재하지 않는다**(코드가 `landing_point_infeasible`로 거절한다).
- **δ를 0으로 몰아도 `R ≤ 0.820m`이 기하학적 절대 상한**이다 — δ 튜닝으로 못 푼다.
- 🔴 **흰 박스는 페인트가 아니다.** `vision_plan.md` §2가 *"정중앙 20cm 흰 사무박스(**안에 나무
  조각상**)"* 라고 적고 있다 — 물리적 상자이고 안에 대회 임무 대상물이 들어있다. 밟으면 임무
  실패다. **하한 제약은 협상 대상이 아니다.**

### 🔴 4-0b. 다음 세션이 이어받을 갭 3건 (에이전트들이 정직하게 보고한 것 중 실재하는 것)

1. **`HOLD`에 종결 경로가 없다.** D의 3분기에서 "이탈 > 임계"는 `HOLD`로 가는데, 재획득이
   안 되면 **무한 호버링**이다(재현 확인: `('HOLD','hold','holding')` 무한 반복). D 세션은
   `FailsafeContract`가 `hold_before_reascend_s`를 "FC가 자기 제어틱에서 재는 값"으로 못박아
   놨다는 이유로 vision 쪽 시한을 만들지 않았고 **그 논거는 옳다** — 다만 **FC 소비자가 아직
   없으므로 지금은 실제로 종결이 안 된다.** 사용자 기조("매끄럽게 보이면")에 직결된다.
2. **`landing_point_infeasible`이 소비자에게 안 간다.** A가 meta 키를 내보내지만 상위 배선이
   없어, 지금은 `detections`가 0이 되어 `main.py`가 기존 `no_target_detection`으로 발행한다 →
   **"안 보임"과 "내려앉을 곳이 없음"이 구분되지 않는다.** C가 `main.py`를 잡고 있어 범위를 뺐다.
3. **`Decision.commit_drift_measured`가 와이어까지 안 간다.** `wire.py` + `shim_core.py` 동시
   수정이 필요해(복제 계약 대조 테스트) D가 `Decision`에서 멈췄다. **오케스트레이터 조율 사항.**

### ⚠️ 4-0c. 오케스트레이터 자신의 검증 실수 (같은 함정을 다음 세션도 밟는다)

C의 "거절 금지" 파괴검증을 처음 돌렸을 때 **1028 passed로 green**이 나와 "테스트 갭"으로 오판할
뻔했다. 원인은 테스트가 아니라 **내 파괴가 no-op이었던 것** — `det.meta`의 키를 `'prior'`로
**추측**해서 썼는데 실제 상수는 `PRIOR_DETECTION_META_KEY = 'prior_geometry'`였다. `.get('prior')`가
None을 반환해 필터가 아무것도 안 걸렀다. 올바른 키로 다시 넣으니 **5 red**.
...
| 🔴 **U6** | **기체 최외곽 반경 `R`의 정확한 값** | 사용자 확인은 **"0.5m 초과"**뿐이고 **사용자가 2026-07-29에 실측 예정**. `R`이 안전창의 폭을 직접 결정하고 **절벽이 0.644m**라 여유가 4.4cm뿐이다(§4-0a). **ψ_m과 같은 성격의 미측정 물리량** — 지어내지 말 것. 아래 U6-a 참조 |
| 🔴 **U6-a** | **"반경"을 무엇으로 재는가** — 현재 코드는 기체를 **단일 반경 원반**으로 뭉갰는데 두 제약이 서로 다른 부위를 본다 | **상한**(매트 이탈→라이즈드 가장자리 전복)은 지면에 닿는 **랜딩기어 접지점 외접원**. **하한**(박스 충돌)은 **높이 0.2m 이하** 구조물의 최대 반경(다리·동체 하부). **프롭이 0.2m보다 높이 달렸으면 프롭은 박스 위를 지나가도 되므로 하한에 안 걸린다** → 단일 R은 과보수적일 수 있고, 분리하면 절벽 문제가 완화될 여지가 있다. 실측 요청 항목: ① 랜딩기어 접지점 외접원 반경 ② 지상고 0.2m 이하 구조물 최대 반경 ③ 프롭 포함 전체 반경 + 프롭 최저점 지상고 |
| 🔴 **U7** | **밀어붙이기 허용 드리프트 `δ`** | 현재 `0.30`(= `distress_box.LANDING_DRIFT_ALLOWANCE_M_DEFAULT`와 같은 값, 갈라지면 테스트 red). 근거 사슬 `M − d − R ≥ δ`가 **미측정 R에 걸려 있어** 잠정값이다(`TERMINAL_COMMIT_DRIFT_MEASURED = False`). `max_drift_estimate_m=0.75`를 그대로 쓰면 **안 된다** — 그건 "데드레코닝을 믿을 수 있는가"이고 δ는 "빗나간 채 내려앉아도 매트 위인가"다 |
| — | **실측 R이 들어오면 고칠 곳** | 두 줄뿐이다: ① `distress_box.AIRCRAFT_RADIUS_M_DEFAULT` ② `state_machine.TERMINAL_COMMIT_DRIFT_MEASURED = False → True`. δ=0.30은 그대로 둔다(`compute_landing_window`가 그 δ를 다시 예산으로 잡아 여유를 재확립한다) |
