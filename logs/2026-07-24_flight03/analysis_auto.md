...
### 고도
- home_position AMSL=364.86m, EKF ref_alt AMSL=366.75m
- AGL 최대 3.38m (t=14.95s), 최소 0.01m (t=0.15s), 종료시 0.9m

### 모드권한 타임라인
| t(s) | nav_state | user_intention | failsafe | arming | 분류 |
|---|---|---|---|---|---|
| 0.51 | 2:POSCTL | 2:POSCTL | 0 | ARMED | AUTO_RECOVERY_OR_UNCLASSIFIED |
...
### 자세
- roll 최대|값|=24.38° (t=12.25s)
- pitch 최대|값|=17.83° (t=9.4s)
- roll 목표(setpoint) 최대|값|=24.84° (t=12.1s) — 이 값이 실측 roll_deg_max_abs 근처까지 크면 세트포인트 자체 결함(순간점프 등) 의심 — 0 근처로 작으면 컨트롤러는 정상 수평을 명령했는데 실제가 못 따라간 것.
- roll rate 이상조짐 후보: t=3.77s (-12.1deg/s)
- pitch rate 이상조짐 후보: t=3.73s (14.0deg/s)
- yaw rate 이상조짐 후보: t=4.69s (-8.2deg/s)
...
### 모터별 커맨드 (0~1)
- 0:전우: mean=0.554 max=0.804 min=0.002
- 1:후좌: mean=0.541 max=0.898 min=0.0
- 2:전좌: mean=0.474 max=0.706 min=0.002
- 3:후우: mean=0.521 max=0.835 min=0.0
- 4:중중: mean=nan max=nan min=nan

### 배터리
- 휴지전압 15.81V, 부하중 최저 12.97V, 전류 최대 25.7A
- remaining: 0.72 → 0.54

### 실패감지기
- imbalanced_prop_metric_max_abs: -3.456
- fd_imbalanced_prop_ever_nonzero: False
- fd_motor_ever_nonzero: False
- motor_failure_mask_ever_nonzero: False

### 이함(liftoff) 순간
- ground_contact 1→0 시점: t=2.6s
